...
namespace AlienEngine.UI.Advanced
{
    public class DataControl : ScrollableControl
    {
        public Skin.LabelSkin LabelSkin { get { return labelSkin; } set { labelSkin = value; if(history.Count > 0) SetData(history.First()); } }
        public Skin.LabelSkin LinkLabelSkin { get { return linkLabelSkin; } set { linkLabelSkin = value; if (history.Count > 0) SetData(history.First()); } }
        ...
        private List<Tuple<Func<object>, Label>> updateData = new List<Tuple<Func<object>, Label>>(256);

        private FlowLayout horizontal, left, right;
        private Skin.LabelSkin labelSkin, linkLabelSkin;

        public DataControl(Gui gui) : base(gui)
        {
            Render += (s, d) => UpdateData();

            horizontal = Add(new FlowLayout(gui) { FlowDirection = FlowDirection.LeftToRight, AutoSize = true });
            left = horizontal.Add(new FlowLayout(gui) { FlowDirection = FlowDirection.TopDown, AutoSize = true });
            var skin = left.Skin;
            skin.Space = 0;
            left.Skin = skin;
            right = horizontal.Add(new FlowLayout(gui) { FlowDirection = FlowDirection.TopDown, AutoSize = true });
            right.Skin = skin;
        ...
        {
			var link = left.Add(new LinkLabel(Gui) { AutoSize = true, Text = name, SkinEnabled = linkLabelSkin });
            if (value() != null)
				link.Click += click;
			var label = right.Add(new Label(Gui) { AutoSize = true, Text = (value() ?? "null").ToString(), SkinEnabled = labelSkin });
            updateData.Add(new Tuple<Func<object>, Label>(value, label));
            row++;
        }
        ...
        MethodInfo updateLayout = typeof(Label).GetMethod("UpdateLayout", BindingFlags.Instance | BindingFlags.InvokeMethod | BindingFlags.NonPublic);
