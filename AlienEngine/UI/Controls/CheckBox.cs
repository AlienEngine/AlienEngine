...
namespace AlienEngine.UI
{
    public class CheckBox : UIContainer
    {
        ...
        public string Text
        {
            ...
            set
            {
                ...
                if (text == null)
                {
                    if (font != null)
                    {
                        text = new Text(Shaders.FontShader, font, textString, UI.Font.Alignement.Left);
                        text.Size = new Sizei(0, this.Size.Y);
                        text.RelativeTo = Corner.BottomLeft;
                        text.Position = new Point2i(UncheckedTexture.Size.Width + 6, Size.Y / 2 - text.TextSize.Y / 2);
        ...
        public CheckBox(Texture uncheckedTexture, Texture checkedTexture, BMFont font, string text)
            : base(new Point2i(0, 0), new Sizei(uncheckedTexture.Size.Width, uncheckedTexture.Size.Height), "CheckBox" + UserInterface.GetUniqueElementID())
        {
            this.UncheckedTexture = uncheckedTexture;
            this.CheckedTexture = checkedTexture;

            checkBox = new Button(UncheckedTexture);
            checkBox.BackgroundColor = new Color4(0, 0, 0, 0);
            checkBox.RelativeTo = Corner.BottomLeft;
            checkBox.Size = new Sizei(uncheckedTexture.Size.Width, uncheckedTexture.Size.Height);
