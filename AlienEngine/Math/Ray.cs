...
namespace AlienEngine
{
    ...
    public struct Ray
    {
        ...
        public bool Intersects(ref BoundingBox boundingBox, out float t)
        {
            ...
            if (Math.Abs(Direction.X) < MathHelper.Epsilon)
            {
        ...
        {
            float velocity;
            Vector3f.Dot(ref Direction, ref plane.Normal, out velocity);
            if (Math.Abs(velocity) < MathHelper.Epsilon)
            {
                t = 0;
                return false;
            }
        ...
        public void GetPointOnRay(float t, out Vector3f v)
        {
