...
namespace AlienEngine.Core.Rendering.Fonts
{
    class FontBuilder
    {
        ...
        public FontBuilder(System.Drawing.Font font, FontBuilderConfiguration config)
        {
        ...
        private List<Sizef> GetGlyphSizes(System.Drawing.Font font)
        {
        ...
        private Bitmap CreateInitialBitmap(System.Drawing.Font font, Sizef maxSize, int initialMargin, out FontGlyph[] glyphs, FontRenderHint renderHint)
        {
        ...
        private static void RetargetGlyphRectangleInwards(BitmapData bitmapData, FontGlyph glyph, bool setYOffset, byte alphaTolerance)
        {
            ...
            if (bitmapData.PixelFormat == PixelFormat.Format32bppArgb)
                emptyPix = delegate(BitmapData data, int x, int y) { return FontBitmap.EmptyAlphaPixel(data, x, y, alphaTolerance); };
            else
                emptyPix = delegate(BitmapData data, int x, int y) { return FontBitmap.EmptyPixel(data, x, y); };
        ...
        private static List<FontBitmap> GenerateBitmapSheetsAndRepack(FontGlyph[] sourceGlyphs, BitmapData[] sourceBitmaps, int destSheetWidth, int destSheetHeight, out FontGlyph[] destGlyphs, int destMargin, bool usePowerOfTwo)
        {
            var pages = new List<FontBitmap>();
            destGlyphs = new FontGlyph[sourceGlyphs.Length];
            FontBitmap currentPage = null;
                ...
                for (int i = 0; i < sourceGlyphs.Length; i++)
                {
                    if(!pre && currentPage == null)
                    {
                        if (finalPageIndex == pages.Count)
                        {
                            ...
                            currentPage = new FontBitmap(new Bitmap(width, height, PixelFormat.Format32bppArgb));
                            currentPage.Clear32(255, 255, 255, 0); // clear to white, but totally transparent
                        }
                        else
                        {
                            currentPage = new FontBitmap(new Bitmap(destSheetWidth, destSheetHeight, PixelFormat.Format32bppArgb));
                            currentPage.Clear32(255, 255, 255, 0); // clear to white, but totally transparent
                        }
                    ...
                    if (xPos + rect.Width + 2 * destMargin <= destSheetWidth && yPos + rect.Height + 2 * destMargin <= destSheetHeight)
                    {
                        if (!pre)
                        {
                            ...
                            if(sourceBitmaps[sourceGlyphs[i].Page].PixelFormat == PixelFormat.Format32bppArgb)
                                FontBitmap.Blit(sourceBitmaps[sourceGlyphs[i].Page], currentPage.bitmapData, rect.X, rect.Y, rect.Width, rect.Height, xPos + destMargin, yPos + destMargin);
                            else
                                FontBitmap.BlitMask(sourceBitmaps[sourceGlyphs[i].Page], currentPage.bitmapData, rect.X, rect.Y, rect.Width, rect.Height, xPos + destMargin, yPos + destMargin);
        ...
        public FontData BuildFontData()
        {
            ...
            var initialBitmapData = initialBmp.LockBits(new System.Drawing.Rectangle(0, 0, initialBmp.Width, initialBmp.Height), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
            ...
            var pages = new List<FontTexture>();
            foreach (var page in bitmapPages)
                pages.Add(new FontTexture(page.bitmapData));
            ...
            fontData.KerningPairs = FontKerningCalculator.CalculateKerning(charSet.ToCharArray(), glyphs, bitmapPages,config.KerningConfig);
        ...
        private static void ScaleSheetsAndGlyphs(List<FontBitmap> pages, FontGlyph[] glyphs, float scale)
        {
        ...
        private static void RetargetAllGlyphs(List<FontBitmap> pages, FontGlyph[] glyphs, byte alphaTolerance)
        {
            foreach (var glyph in glyphs)
                RetargetGlyphRectangleOutwards(pages[glyph.Page].bitmapData, glyph, false, alphaTolerance);
        }

        public static void CreateBitmapPerGlyph(FontGlyph[] sourceGlyphs, FontBitmap[] sourceBitmaps, out FontGlyph[]  destGlyphs, out FontBitmap[] destBitmaps)
        {
            destBitmaps = new FontBitmap[sourceGlyphs.Length];
            destGlyphs = new FontGlyph[sourceGlyphs.Length];
            for(int i = 0; i < sourceGlyphs.Length; i++)
            {
                var sg = sourceGlyphs[i];
                destGlyphs[i] = new FontGlyph(i,new Rectangle(0,0,sg.Rect.Width,sg.Rect.Height),sg.YOffset,sg.Character);
                destBitmaps[i] = new FontBitmap(new Bitmap(sg.Rect.Width,sg.Rect.Height,PixelFormat.Format32bppArgb));
                FontBitmap.Blit(sourceBitmaps[sg.Page].bitmapData,destBitmaps[i].bitmapData,sg.Rect,0,0);
            }
        }
