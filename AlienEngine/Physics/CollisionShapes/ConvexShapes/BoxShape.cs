...
namespace AlienEngine.Core.Physics.CollisionShapes.ConvexShapes
{
    ...
    public class BoxShape : ConvexShape
    {
        ...
        public static ConvexShapeDescription ComputeDescription(float width, float height, float length, float collisionMargin)
        {
            ...
            description.EntityShapeVolume.VolumeDistribution = new Matrix3f();
        ...
        {
            extremePoint = new Vector3f(Math.Sign(direction.X) * (halfWidth - collisionMargin), Math.Sign(direction.Y) * (halfHeight - collisionMargin), Math.Sign(direction.Z) * (halfLength - collisionMargin));
        }
        ...
        public override bool RayTest(ref Ray ray, ref RigidTransform transform, float maximumLength, out RayHit hit)
        {
            ...
            Vector3f.Subtract(ref ray.Position, ref transform.Position, out localOrigin);
            Vector3f.Transform(ref localOrigin, ref conjugate, out localOrigin);
            Vector3f localDirection;
            Vector3f.Transform(ref ray.Direction, ref conjugate, out localDirection);
            Vector3f normal = Toolbox.ZeroVector;
            float temp, tmin = 0, tmax = maximumLength;

            if (Math.Abs(localDirection.X) < MathHelper.Epsilon && (localOrigin.X < -halfWidth || localOrigin.X > halfWidth))
                return false;
            ...
            var tempNormal = new Vector3f(-1, 0, 0);
            if (t1 > t2)
            {
                temp = t1;
                t1 = t2;
            ...
            if (tmin > tmax)
                return false;
            if (Math.Abs(localDirection.Y) < MathHelper.Epsilon && (localOrigin.Y < -halfHeight || localOrigin.Y > halfHeight))
                return false;
            ...
            if (tmin > tmax)
                return false;
            if (Math.Abs(localDirection.Z) < MathHelper.Epsilon && (localOrigin.Z < -halfLength || localOrigin.Z > halfLength))
                return false;
            ...
            if (tmin > tmax)
                return false;
            hit.T = tmin;
            Vector3f.Multiply(ref ray.Direction, tmin, out hit.Location);
