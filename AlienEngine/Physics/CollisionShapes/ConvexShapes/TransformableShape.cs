...
namespace AlienEngine.Core.Physics.CollisionShapes.ConvexShapes
{
    ...
    public class TransformableShape : ConvexShape
    {
        ...
        {
            Vector3f d;
            Vector3f.TransformTranspose(ref direction, ref transform, out d);
            shape.GetLocalExtremePoint(d, out extremePoint);
            Vector3f.Transform(ref extremePoint, ref transform, out extremePoint);
        }
        ...
        public float ComputeMaximumRadius()
        {
            ...
            Vector3f diameter;
            Vector3f.Subtract(ref boundingBox.Max, ref boundingBox.Min, out diameter);
