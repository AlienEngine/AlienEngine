...
namespace AlienEngine.Core.Physics.CollisionShapes.ConvexShapes
{
    ///<summary>
    ...
    public static class InertiaHelper
    {
        ...
        public static void GetPointContribution(float pointWeight, ref Vector3f center, ref Vector3f point, out Matrix3f contribution)
        {
            Vector3f.Subtract(ref point, ref center, out point);
        ...
        /// </summary>
        public static Vector3f[] SampleDirections;
        ...
        public static void GenerateSphere(int subdivisionCount, out Vector3f[] vertices, out int[] triangles)
        {
            ...
            vertices = new Vector3f[GetExpectedVertexCount(subdivisionCount)];

            ////Create the regular tetrahedron vertices.
            //float x = (float)(1 / Math.Sqrt(3));
            //float z = (float)(-1 / (2 * Math.Sqrt(6)));
            //vertices[0] = Vector3f.Normalize(new Vector3f(0, 0, (float)(Math.Sqrt(2.0 / 3.0) + z)));
            //vertices[1] = Vector3f.Normalize(new Vector3f(-0.5f * x, -0.5f, z));
            //vertices[2] = Vector3f.Normalize(new Vector3f(-0.5f * x, 0.5f, z));
            //vertices[3] = Vector3f.Normalize(new Vector3f(x, 0, z));
            ...
            vertices[0] = new Vector3f(0, x, y);
            vertices[1] = new Vector3f(0, -x, y);
            vertices[2] = new Vector3f(0, x, -y);
            vertices[3] = new Vector3f(0, -x, -y);

            vertices[4] = new Vector3f(x, y, 0);
            vertices[5] = new Vector3f(-x, y, 0);
            vertices[6] = new Vector3f(x, -y, 0);
            vertices[7] = new Vector3f(-x, -y, 0);

            vertices[8] = new Vector3f(y, 0, x);
            vertices[9] = new Vector3f(-y, 0, x);
            vertices[10] = new Vector3f(y, 0, -x);
            vertices[11] = new Vector3f(-y, 0, -x);
            ...
            for (int i = 0; i < subdivisionCount; ++i)
            {
                for (int triangleIndex = 0; triangleIndex < currentTriangleIndexCount; triangleIndex += 3)
                {
                    ...
                    if (!edges.TryGetValue(edge, out abMidIndex))
                    {
                        ...
                        //Create a vertex.
                        Vector3f mid;
                        Vector3f.Add(ref a, ref b, out mid);
                        mid.Normalize();
                    ...
                    if (!edges.TryGetValue(edge, out bcMidIndex))
                    {
                        ...
                        //Create a vertex.
                        Vector3f mid;
                        Vector3f.Add(ref b, ref c, out mid);
                        mid.Normalize();
                    ...
                    if (!edges.TryGetValue(edge, out acMidIndex))
                    {
                        //This edge hasn't yet been handled by another triangle.
                        Vector3f mid;
                        Vector3f.Add(ref a, ref c, out mid);
                        mid.Normalize();
            ...
            //RawList<Vector3f> surfacePoints = new RawList<Vector3f>();
        ...
        public static void ComputeShapeDistribution(IList<Vector3f> vertices, IList<int> triangleIndices, out float volume, out Matrix3f volumeDistribution)
        {
            ...
            {
                Vector3f v2 = vertices[triangleIndices[i]];
                Vector3f v3 = vertices[triangleIndices[i + 1]];
                Vector3f v4 = vertices[triangleIndices[i + 2]];
            ...
            bo *= offFactor;
            co *= offFactor;
            volumeDistribution = new Matrix3f(a, bo, co,
                                               bo, b, ao,
                                               co, ao, c);
        ...
        /// <param name="volume">Volume of the shape.</param>
        /// <param name="volumeDistribution">Distribution of the volume as measured from the computed center.</param>
        public static void ComputeShapeDistribution(IList<Vector3f> vertices, IList<int> triangleIndices, out Vector3f center, out float volume, out Matrix3f volumeDistribution)
        {
            //Explanation for the tetrahedral integration bits: Explicit Exact Formulas for the 3-D Tetrahedron Inertia Tensor in Terms of its Vertex Coordinates
            //http://www.scipub.org/fulltext/jms2/jms2118-11.pdf
            //x1, x2, x3, x4 are origin, triangle1, triangle2, triangle3
            ...
            Vector3f summedCenter = new Vector3f();
            float scaledVolume = 0;
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                Vector3f v2 = vertices[triangleIndices[i]];
                ...
                Vector3f tetrahedronCentroid;
                Vector3f.Add(ref v2, ref v3, out tetrahedronCentroid);
                Vector3f.Add(ref tetrahedronCentroid, ref v4, out tetrahedronCentroid);
                Vector3f.Multiply(ref tetrahedronCentroid, scaledTetrahedronVolume, out tetrahedronCentroid);
                Vector3f.Add(ref tetrahedronCentroid, ref summedCenter, out summedCenter);
            ...
            if (scaledVolume < MathHelper.Epsilon)
            {
            ...
            {
                Vector3f.Multiply(ref summedCenter, 0.25f / scaledVolume, out center);
                volume = scaledVolume / 6;
                float scaledDensity = 1 / volume;
                float diagonalFactor = scaledDensity / 60;
                float offFactor = -scaledDensity / 120;
                ...
                //                                   bo, b, ao,
                //                                   co, ao, c);
                volumeDistribution = new Matrix3f(a, co, bo,
                                                   co, b, ao,
                                                   bo, ao, c);
                ...
                Matrix3f.Subtract(ref volumeDistribution, ref additionalInertia, out volumeDistribution);
        ...
        //public static void GetInertiaOffset(Vector3f offset, float mass, out Matrix3f additionalInertia)
        //{
        ...
        public static float ComputeMinimumRadius(IList<Vector3f> vertices, IList<int> triangleIndices, ref Vector3f center)
        {
            ...
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                ...
                Vector3f.Subtract(ref v4, ref v2, out v2v4);
                Vector3f normal;
                Vector3f.Cross(ref v2v4, ref v2v3, out normal);
                ...
                if (lengthSquared > 1e-10f)
                    Vector3f.Divide(ref normal, (float)Math.Sqrt(lengthSquared), out normal);
                else
                    continue;

                Vector3f fromCenterToPlane;
                Vector3f.Subtract(ref v2, ref center, out fromCenterToPlane);

                float distance;
                Vector3f.Dot(ref normal, ref fromCenterToPlane, out distance);
                if (distance < 0)
