...
namespace AlienEngine.Core.Physics.CollisionShapes.ConvexShapes
{
    ...
    public class TriangleShape : ConvexShape
    {
        ...
        public TriangleShape(Vector3f vA, Vector3f vB, Vector3f vC)
        {
            //Recenter.  Convexes should contain the origin.
            Vector3f center = (vA + vB + vC) / 3;
            this.vA = vA - center;
            this.vB = vB - center;
            this.vC = vC - center;
        ...
        public TriangleShape(Vector3f vA, Vector3f vB, Vector3f vC, out Vector3f center)
        {
            //Recenter.  Convexes should contain the origin.
            center = (vA + vB + vC) / 3;
            this.vA = vA - center;
        ...
        public TriangleShape(Vector3f vA, Vector3f vB, Vector3f vC, ConvexShapeDescription description)
        {
            //Recenter.  Convexes should contain the origin.
        ...
        public static ConvexShapeDescription ComputeDescription(Vector3f vA, Vector3f vB, Vector3f vC, float collisionMargin)
        {
            ...
            var doubleArea = Vector3f.Cross(vB - vA, vC - vA).Length;
            ...
            Matrix3f.MultiplyTransposed(ref v, ref s, out description.EntityShapeVolume.VolumeDistribution);
            Matrix3f.Multiply(ref description.EntityShapeVolume.VolumeDistribution, ref v, out description.EntityShapeVolume.VolumeDistribution);
            var scaling = doubleArea / 24f;
            Matrix3f.Multiply(ref description.EntityShapeVolume.VolumeDistribution, -scaling, out description.EntityShapeVolume.VolumeDistribution);
        ...
        public override void GetLocalExtremePointWithoutMargin(ref Vector3f direction, out Vector3f extremePoint)
        {
            ...
            Vector3f.Dot(ref direction, ref vA, out dotA);
            Vector3f.Dot(ref direction, ref vB, out dotB);
            Vector3f.Dot(ref direction, ref vC, out dotC);
        ...
        public static Matrix3f ComputeVolumeDistribution(Vector3f vA, Vector3f vB, Vector3f vC)
        {
            Vector3f center = (vA + vB + vC) * (1 / 3f);
            ...
            var volumeDistribution = new Matrix3f(massPerPoint * (j * j + k * k), massPerPoint * (-i * j), massPerPoint * (-i * k),
            ...
            j = vB.Y - center.Y;
            k = vB.Z - center.Z;
            var pointContribution = new Matrix3f(massPerPoint * (j * j + k * k), massPerPoint * (-i * j), massPerPoint * (-i * k),
                                                  massPerPoint * (-i * j), massPerPoint * (i * i + k * k), massPerPoint * (-j * k),
                                                  massPerPoint * (-i * k), massPerPoint * (-j * k), massPerPoint * (i * i + j * j));
            Matrix3f.Add(ref volumeDistribution, ref pointContribution, out volumeDistribution);

            i = vC.X - center.X;
            j = vC.Y - center.Y;
            k = vC.Z - center.Z;
            pointContribution = new Matrix3f(massPerPoint * (j * j + k * k), massPerPoint * (-i * j), massPerPoint * (-i * k),
                                              massPerPoint * (-i * j), massPerPoint * (i * i + k * k), massPerPoint * (-j * k),
                                              massPerPoint * (-i * k), massPerPoint * (-j * k), massPerPoint * (i * i + j * j));
            Matrix3f.Add(ref volumeDistribution, ref pointContribution, out volumeDistribution);
        ...
        public Vector3f GetLocalNormal()
        {
            Vector3f normal;
            Vector3f vAvB;
            Vector3f vAvC;
            ...
            Vector3f.Cross(ref vAvB, ref vAvC, out normal);
            normal.Normalize();
            return normal;
        }
        ...
        public Vector3f GetNormal(RigidTransform transform)
        {
            Vector3f normal = GetLocalNormal();
            Vector3f.Transform(ref normal, ref transform.Orientation, out normal);
            return normal;
        }
        ...
        public override bool RayTest(ref Ray ray, ref RigidTransform transform, float maximumLength, out RayHit hit)
        {
            ...
            Matrix3f.FromQuaternion(ref transform.Orientation, out orientation);
            Ray localRay;
