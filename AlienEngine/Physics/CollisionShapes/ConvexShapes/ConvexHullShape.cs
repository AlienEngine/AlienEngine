...
namespace AlienEngine.Core.Physics.CollisionShapes.ConvexShapes
{
    ...
    public class ConvexHullShape : ConvexShape
    {
        ...
        public ReadOnlyList<Vector3f> Vertices
        {
            get
            {
                return new ReadOnlyList<Vector3f>(vertices);
            }
        }
        Vector3f[] vertices;
        ...
        public ConvexHullShape(IList<Vector3f> vertices)
        {
            if (vertices.Count == 0)
        ...
        ///<exception cref="ArgumentException">Thrown when the point set is empty.</exception>
        public ConvexHullShape(IList<Vector3f> vertices, out Vector3f center)
        {
            if (vertices.Count == 0)
                throw new ArgumentException("Vertices list used to create a ConvexHullShape cannot be empty.");
        ...
        ///<exception cref="ArgumentException">Thrown when the point set is empty.</exception>
        public ConvexHullShape(IList<Vector3f> vertices, out Vector3f center, IList<int> outputHullTriangleIndices, IList<Vector3f> outputUniqueSurfaceVertices)
        {
            if (vertices.Count == 0)
                throw new ArgumentException("Vertices list used to create a ConvexHullShape cannot be empty.");
            ...
            this.vertices = new Vector3f[outputUniqueSurfaceVertices.Count];
        ...
        public ConvexHullShape(IList<Vector3f> localSurfaceVertices, ConvexShapeDescription description)
        {
            ...
            vertices = new Vector3f[localSurfaceVertices.Count];
        ...
        public static ConvexShapeDescription ComputeDescription(IList<Vector3f> vertices, float collisionMargin, out Vector3f center, IList<int> outputHullTriangleIndices, IList<Vector3f> outputUniqueSurfaceVertices)
        {
        ...
        public static float ComputeMaximumRadius(IList<Vector3f> localSurfaceVertices, float collisionMargin)
        {
        ...
        public override void GetBoundingBox(ref RigidTransform shapeTransform, out BoundingBox boundingBox)
        {
            ...
            float minY, maxY;
            float minZ, maxZ;
            var right = new Vector3f(o.M11, o.M21, o.M31);
            var up = new Vector3f(o.M12, o.M22, o.M32);
            var backward = new Vector3f(o.M13, o.M23, o.M33);
            Vector3f.Dot(ref vertices[0], ref right, out maxX);
            minX = maxX;
            Vector3f.Dot(ref vertices[0], ref up, out maxY);
            minY = maxY;
            Vector3f.Dot(ref vertices[0], ref backward, out maxZ);
            minZ = maxZ;
            int minXIndex = 0;
            int maxXIndex = 0;
            int minYIndex = 0;
            ...
            {
                float dot;
                Vector3f.Dot(ref vertices[i], ref right, out dot);
                if (dot < minX)
                {
                    minX = dot;
                    minXIndex = i;
                ...
                Vector3f.Dot(ref vertices[i], ref up, out dot);
                if (dot < minY)
                {
                    minY = dot;
                    minYIndex = i;
                ...
                Vector3f.Dot(ref vertices[i], ref backward, out dot);
                if (dot < minZ)
                {
                    minZ = dot;
                    minZIndex = i;
        ...
        {
            float max;
            Vector3f.Dot(ref vertices[0], ref direction, out max);
            int maxIndex = 0;
            for (int i = 1; i < vertices.Length; i++)
            {
                float dot;
                Vector3f.Dot(ref vertices[i], ref direction, out dot);
                if (dot > max)
                {
                    max = dot;
                    maxIndex = i;
