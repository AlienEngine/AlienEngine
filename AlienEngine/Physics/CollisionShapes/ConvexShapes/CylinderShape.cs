...
namespace AlienEngine.Core.Physics.CollisionShapes.ConvexShapes
{
    ...
    public class CylinderShape : ConvexShape
    {
        ...
        public override void GetBoundingBox(ref RigidTransform shapeTransform, out BoundingBox boundingBox)
        {
            ...
            var direction = new Vector3f(o.M11, o.M21, o.M31);
            Vector3f right;
            GetLocalExtremePointWithoutMargin(ref direction, out right);

            direction = new Vector3f(o.M12, o.M22, o.M32);
            Vector3f up;
            GetLocalExtremePointWithoutMargin(ref direction, out up);

            direction = new Vector3f(o.M13, o.M23, o.M33);
            Vector3f backward;
        ...
        public override void GetLocalExtremePointWithoutMargin(ref Vector3f direction, out Vector3f extremePoint)
        {
            ...
            if (horizontalLengthSquared > MathHelper.Epsilon)
            {
                float multiplier = (radius - collisionMargin) / (float)Math.Sqrt(horizontalLengthSquared);
                extremePoint = new Vector3f(direction.X * multiplier, Math.Sign(direction.Y) * (halfHeight - collisionMargin), direction.Z * multiplier);
            }
            else
            {
                extremePoint = new Vector3f(0, Math.Sign(direction.Y) * (halfHeight - collisionMargin), 0);
            }
        ...
        upperTest:
            ...
            Vector3f.Multiply(ref localRay.Direction, t, out planeIntersection);
            Vector3f.Add(ref localRay.Position, ref planeIntersection, out planeIntersection);
            if(planeIntersection.X * planeIntersection.X + planeIntersection.Z * planeIntersection.Z < radius * radius + 1e-9 && t < maximumLength)
            {
                //Pull the hit into world space.
                Vector3f.Transform(ref Toolbox.UpVector, ref transform.Orientation, out hit.Normal);
        ...
        lowerTest:
            ...
            {
                //Pull the hit into world space.
                Vector3f.Transform(ref Toolbox.DownVector, ref transform.Orientation, out hit.Normal);
                RigidTransform.Transform(ref planeIntersection, ref transform, out hit.Location);
                hit.T = t;
                return true;
            }
