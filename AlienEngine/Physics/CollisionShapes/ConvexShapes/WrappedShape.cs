...
namespace AlienEngine.Core.Physics.CollisionShapes.ConvexShapes
{
    ...
    public struct ConvexShapeEntry
    {
        ...
        public ConvexShapeEntry(Vector3f position, ConvexShape shape)
        {
    ...
    public class WrappedShape : ConvexShape
    {
        ...
        public WrappedShape(ConvexShapeEntry firstShape, ConvexShapeEntry secondShape, out Vector3f center)
        {
            shapes.Add(firstShape);
            shapes.Add(secondShape);
        ...
        /// <param name="center">Center of the shape before recentering.</param>
        ///<exception cref="Exception">Thrown when the shape list is empty.</exception>
        public WrappedShape(IList<ConvexShapeEntry> shapeEntries, out Vector3f center)
        {
            if (shapeEntries.Count == 0)
                throw new ArgumentException("Cannot create a wrapped shape with no contained shapes.");
            for (int i = 0; i < shapeEntries.Count; i++)
        ...
        public void UpdateConvexShapeInfo(out Vector3f center)
        {
        ...
        public override void GetLocalExtremePointWithoutMargin(ref Vector3f direction, out Vector3f extremePoint)
        {
            ...
            float maxDot;
            Vector3f.Dot(ref extremePoint, ref direction, out maxDot);
            for (int i = 1; i < shapes.WrappedList.Count; i++)
            {
                float dot;
                Vector3f temp;
                ...
                Vector3f.Dot(ref direction, ref temp, out dot);
                if (dot > maxDot)
                {
                    extremePoint = temp;
                    maxDot = dot;
