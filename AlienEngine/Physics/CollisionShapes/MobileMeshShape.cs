...
namespace AlienEngine.Core.Physics.CollisionShapes
{
    ///<summary>
    ...
    public class MobileMeshShape : EntityShape
    {
        ...
        RawList<Vector3f> hullVertices = new RawList<Vector3f>();

        /// <summary>
        /// Gets the list of vertices on the convex hull of the mesh used to compute the bounding box.
        /// </summary>
        public ReadOnlyList<Vector3f> HullVertices
        {
            get { return new ReadOnlyList<Vector3f>(hullVertices); }
        }
        ...
        public MobileMeshShape(Vector3f[] vertices, int[] indices, AffineTransform localTransform, MobileMeshSolidity solidity)
        {
        ...
        public MobileMeshShape(Vector3f[] vertices, int[] indices, AffineTransform localTransform, MobileMeshSolidity solidity, out Vector3f center)
        {
            this.solidity = solidity;
            var data = new TransformableMeshData(vertices, indices, localTransform);
            var shapeDistributionInformation = ComputeVolumeDistribution(data);
        ...
        TriangleSidedness ComputeSolidSidednessHelper(Ray ray)
        {
            ...
            if (triangleMesh.Tree.GetOverlaps(ray, hitList))
            {
                ...
                if (hits.Count % 2 == 0)
                {
                    ...
                    var normal = Vector3f.Cross(vA - vB, vA - vC);
        ...
        private void UpdateSurfaceVertices()
        {
            ...
            else
            {
                ...
                hullVertices.Add(new Vector3f(box.Min.X, box.Min.Y, box.Max.Z));
                hullVertices.Add(new Vector3f(box.Min.X, box.Max.Y, box.Min.Z));
                hullVertices.Add(new Vector3f(box.Max.X, box.Min.Y, box.Min.Z));
                hullVertices.Add(new Vector3f(box.Min.X, box.Max.Y, box.Max.Z));
                hullVertices.Add(new Vector3f(box.Max.X, box.Max.Y, box.Min.Z));
                hullVertices.Add(new Vector3f(box.Max.X, box.Min.Y, box.Max.Z));
            }
        }
        ...
        private ShapeDistributionInformation ComputeVolumeDistribution(TransformableMeshData data)
        {
            ...
            shapeInformation.Center = new Vector3f();
            shapeInformation.VolumeDistribution = new Matrix3f();
            float totalWeight = 0;
            for (int i = 0; i < data.indices.Length; i += 3)
            {
                ...
                Vector3f vAvB;
                Vector3f vAvC;
                Vector3f.Subtract(ref vB, ref vA, out vAvB);
                Vector3f.Subtract(ref vC, ref vA, out vAvC);
                Vector3f cross;
                Vector3f.Cross(ref vAvB, ref vAvC, out cross);
                ...
                Matrix3f aContribution, bContribution, cContribution;
                ...
                Matrix3f.Add(ref aContribution, ref shapeInformation.VolumeDistribution, out shapeInformation.VolumeDistribution);
                Matrix3f.Add(ref bContribution, ref shapeInformation.VolumeDistribution, out shapeInformation.VolumeDistribution);
                Matrix3f.Add(ref cContribution, ref shapeInformation.VolumeDistribution, out shapeInformation.VolumeDistribution);
            ...
            Matrix3f.Multiply(ref shapeInformation.VolumeDistribution, 1 / (2 * totalWeight), out shapeInformation.VolumeDistribution);

            //Move the inertia tensor into position according to the center.
            Matrix3f additionalInertia;
            InertiaHelper.GetPointContribution(0.5f, ref Toolbox.ZeroVector, ref shapeInformation.Center, out additionalInertia);
            Matrix3f.Subtract(ref shapeInformation.VolumeDistribution, ref additionalInertia, out shapeInformation.VolumeDistribution);
        ...
        private void GetBoundingBox(ref Matrix3f o, out BoundingBox boundingBox)
        {
#if !WINDOWS
            boundingBox = new BoundingBox();
#endif
            //Sample the local directions from the matrix, implicitly transposed.
            var rightDirection = new Vector3f(o.M11, o.M21, o.M31);
            var upDirection = new Vector3f(o.M12, o.M22, o.M32);
            var backDirection = new Vector3f(o.M13, o.M23, o.M33);
            ...
            for (int i = 0; i < hullVertices.Count; i++)
            {
                ...
                Vector3f.Dot(ref rightDirection, ref hullVertices.Elements[i], out dotX);
                Vector3f.Dot(ref upDirection, ref hullVertices.Elements[i], out dotY);
                Vector3f.Dot(ref backDirection, ref hullVertices.Elements[i], out dotZ);
                if (dotX < minX)
                {
                    minX = dotX;
                    left = i;
            ...
            Vector3f.Multiply(ref rightDirection, meshCollisionMargin / (float)Math.Sqrt(rightDirection.Length), out rightDirection);
            Vector3f.Multiply(ref upDirection, meshCollisionMargin / (float)Math.Sqrt(upDirection.Length), out upDirection);
            Vector3f.Multiply(ref backDirection, meshCollisionMargin / (float)Math.Sqrt(backDirection.Length), out backDirection);
            ...
            Vector3f.Add(ref rightElement, ref rightDirection, out rightElement);
            Vector3f.Subtract(ref leftElement, ref rightDirection, out leftElement);
            Vector3f.Add(ref upElement, ref upDirection, out upElement);
            Vector3f.Subtract(ref downElement, ref upDirection, out downElement);
            Vector3f.Add(ref backwardElement, ref backDirection, out backwardElement);
            Vector3f.Subtract(ref forwardElement, ref backDirection, out forwardElement);
