...
namespace AlienEngine.Core.Physics.CollisionShapes
{
    ...
    public class TerrainShape : CollisionShape
    {
        ...
        public void GetBoundingBox(ref AffineTransform transform, out BoundingBox boundingBox)
        {
            ...
            Vector3f minXvertex = new Vector3f(),
                    maxXvertex = new Vector3f(),
                    minYvertex = new Vector3f(),
                    maxYvertex = new Vector3f(),
                    minZvertex = new Vector3f(),
                    maxZvertex = new Vector3f();
            ...
            for (int i = 0; i < heights.GetLength(0); i++)
            {
                ...
                {
                    var vertex = new Vector3f(i, heights[i, j], j);
                    Vector3f.Transform(ref vertex, ref transform.LinearTransform, out vertex);
                    if (vertex.X < minX)
                    {
                        minX = vertex.X;
                        minXvertex = vertex;
        ...
        public bool RayCast(ref Ray ray, float maximumLength, ref AffineTransform transform, TriangleSidedness sidedness, out RayHit hit)
        {
            ...
            Vector3f.Transform(ref ray.Direction, ref inverse.LinearTransform, out localRay.Direction);
            ...
            Vector3f progressingOrigin = localRay.Position;
            float distance = 0;
            ...
            if (progressingOrigin.X < 0)
            {
                if (localRay.Direction.X > 0)
                {
                    ...
                    distance += timeToMinX;
                    Vector3f increment;
                    Vector3f.Multiply(ref localRay.Direction, timeToMinX, out increment);
                    Vector3f.Add(ref increment, ref progressingOrigin, out progressingOrigin);
                }
                else
            ...
            if (progressingOrigin.Z < 0)
            {
                if (localRay.Direction.Z > 0)
                {
                    ...
                    distance += timeToMinZ;
                    Vector3f increment;
                    Vector3f.Multiply(ref localRay.Direction, timeToMinZ, out increment);
                    Vector3f.Add(ref increment, ref progressingOrigin, out progressingOrigin);
                }
                else
                    return false;
            ...
            while (true)
            {
                    ...
                    progressingOrigin.Y < lowest && localRay.Direction.Y < 0))
                {
                    ...
                    if (didHit1 && didHit2)
                    {
                        if (hit1.T < hit2.T)
                        {
                            Vector3f.Multiply(ref ray.Direction, hit1.T, out hit.Location);
                            Vector3f.Add(ref hit.Location, ref ray.Position, out hit.Location);
                            Vector3f.TransformTranspose(ref hit1.Normal, ref inverse.LinearTransform, out hit.Normal);
                            hit.T = hit1.T;
                            return true;
                        }
                        Vector3f.Multiply(ref ray.Direction, hit2.T, out hit.Location);
                        Vector3f.Add(ref hit.Location, ref ray.Position, out hit.Location);
                        Vector3f.TransformTranspose(ref hit2.Normal, ref inverse.LinearTransform, out hit.Normal);
                        hit.T = hit2.T;
                        return true;
                    }
                    else if (didHit1)
                ...
                if (timeToX < timeToZ)
                {
                    ...
                    Vector3f increment;
                    Vector3f.Multiply(ref localRay.Direction, timeToX, out increment);
                    Vector3f.Add(ref increment, ref progressingOrigin, out progressingOrigin);
                }
                else
                {
                    ...
                    Vector3f increment;
                    Vector3f.Multiply(ref localRay.Direction, timeToZ, out increment);
                    Vector3f.Add(ref increment, ref progressingOrigin, out progressingOrigin);
                }
        ...
        public void GetLocalPosition(int columnIndex, int rowIndex, out Vector3f v)
        {
#if !WINDOWS
            v = new Vector3f();
#endif
        ...
        public void GetPosition(int columnIndex, int rowIndex, ref AffineTransform transform, out Vector3f position)
        {
            if (columnIndex <= 0)
                columnIndex = 0;
        ...
        public void GetLocalNormal(int columnIndex, int rowIndex, out Vector3f normal)
        {
            ...
            //Since the horizontal offsets are known to be 1 in local space, we can omit quite a few operations compared to a full Vector3f and cross product.

            //The implicit vectors are:
            //var leftToRight = new Vector3f(2, rightHeight - leftHeight, 0);
            //var bottomToTop = new Vector3f(0, topHeight - bottomHeight, 2);
            //the result is then:
            //Vector3f.Cross(bottomToTop, leftToRight);
            //Which is:
        ...
        public void GetFirstTriangle(int columnIndex, int rowIndex, ref AffineTransform transform, out Vector3f a, out Vector3f b, out Vector3f c)
        {
            if (quadTriangleOrganization == QuadTriangleOrganization.BottomLeftUpperRight)
            {
        ...
        ///<param name="b">Second vertex of the triangle.</param>
        ///<param name="c">Third vertex of the triangle.</param>
        public void GetSecondTriangle(int columnIndex, int rowIndex, ref AffineTransform transform, out Vector3f a, out Vector3f b, out Vector3f c)
        {
            if (quadTriangleOrganization == QuadTriangleOrganization.BottomLeftUpperRight)
            {
        ...
        ///<param name="b">Second vertex of the triangle.</param>
        ///<param name="c">Third vertex of the triangle.</param>
        public void GetTriangle(int index, ref AffineTransform transform, out Vector3f a, out Vector3f b, out Vector3f c)
        {
            //Find the quad.
