...
namespace AlienEngine.Core.Physics.Utilities
{
    ...
    public static partial class ConvexHullHelper
    {
        ...
        public static void GetConvexHull(IList<Vector3f> points, IList<int> outputTriangleIndices)
        {
        ...
        public static void GetConvexHull(IList<Vector3f> points, IList<Vector3f> outputSurfacePoints)
        {
            var rawPoints = CommonResources.GetVectorList();
            rawPoints.AddRange(points);
        ...
        /// <param name="points">List of points in the set.</param>
        /// <param name="outputSurfacePoints">Unique points on the surface of the convex hull.</param>
        public static void GetConvexHull(RawList<Vector3f> points, IList<Vector3f> outputSurfacePoints)
        {
        ...
        /// Each group of 3 indices represents a triangle on the surface of the hull.</param>
        /// <param name="outputSurfacePoints">Unique points on the surface of the convex hull.</param>
        public static void GetConvexHull(IList<Vector3f> points, IList<int> outputTriangleIndices, IList<Vector3f> outputSurfacePoints)
        {
            var rawPoints = CommonResources.GetVectorList();
            var rawIndices = CommonResources.GetIntList();
            rawPoints.AddRange(points);
        ...
        /// Each group of 3 indices represents a triangle on the surface of the hull.</param>
        /// <param name="outputSurfacePoints">Unique points on the surface of the convex hull.</param>
        public static void GetConvexHull(RawList<Vector3f> points, RawList<int> outputTriangleIndices, IList<Vector3f> outputSurfacePoints)
        {
        ...
        /// <param name="outputTriangleIndices">List of indices into the input point set composing the triangulated surface of the convex hull.
        /// Each group of 3 indices represents a triangle on the surface of the hull.</param>
        public static void GetConvexHull(RawList<Vector3f> points, RawList<int> outputTriangleIndices)
        {
            if (points.Count == 0)
            {
            ...
            while (outsidePoints.Count > 0)
            {
                ...
                for (int k = 0; k < outputTriangleIndices.Count; k += 3)
                {
                    ...
                    Vector3f maximum = points.Elements[maxIndex];

                    //If the point is beyond the current triangle, continue.
                    Vector3f offset;
                    Vector3f.Subtract(ref maximum, ref points.Elements[outputTriangleIndices.Elements[k]], out offset);
                    float dot;
        ...
        private static int GetExtremePoint(ref Vector3f direction, RawList<Vector3f> points, RawList<int> outsidePoints)
        {
            ...
            {
                float dot;
                Vector3f.Dot(ref points.Elements[outsidePoints[i]], ref direction, out dot);
                if (dot > maximumDot)
                {
                    maximumDot = dot;
                    extremeIndex = i;
        ...
        private static void GetExtremePoints(ref Vector3f direction, RawList<Vector3f> points, out float maximumDot, out float minimumDot, out int maximumIndex, out int minimumIndex)
        {
            maximumIndex = 0;
            minimumIndex = 0;

            float dot;
            Vector3f.Dot(ref points.Elements[0], ref direction, out dot);
            minimumDot = dot;
            maximumDot = dot;
            for (int i = 1; i < points.Count; ++i)
            {
                Vector3f.Dot(ref points.Elements[i], ref direction, out dot);
                if (dot > maximumDot)
                {
                    maximumDot = dot;
                    maximumIndex = i;
        ...
        private static void ComputeInitialTetrahedron(RawList<Vector3f> points, RawList<int> outsidePointCandidates, RawList<int> triangleIndices, out Vector3f centroid)
        {
            ...
            Vector3f ab;
            Vector3f.Subtract(ref points.Elements[b], ref points.Elements[a], out ab);
            Vector3f.Cross(ref ab, ref Toolbox.UpVector, out direction);
            if (direction.LengthSquared < MathHelper.Epsilon)
                Vector3f.Cross(ref ab, ref Toolbox.RightVector, out direction);
            ...
            float dot;
            Vector3f.Dot(ref direction, ref points.Elements[a], out dot);
            ...
            Vector3f ac;
            Vector3f.Subtract(ref points.Elements[c], ref points.Elements[a], out ac);
            Vector3f.Cross(ref ab, ref ac, out direction);
            ...
            Vector3f.Add(ref points.Elements[a], ref points.Elements[b], out centroid);
            Vector3f.Add(ref centroid, ref points.Elements[c], out centroid);
            Vector3f.Add(ref centroid, ref points.Elements[d], out centroid);
            Vector3f.Multiply(ref centroid, 0.25f, out centroid);
            ...
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                ...
                Vector3f cross;
                Vector3f.Subtract(ref vB, ref vA, out ab);
                Vector3f.Subtract(ref vC, ref vA, out ac);
                Vector3f.Cross(ref ac, ref ab, out cross);
                Vector3f offset;
                Vector3f.Subtract(ref vA, ref centroid, out offset);
                float volume;
                Vector3f.Dot(ref offset, ref cross, out volume);
                //This volume/cross product could also be used to check for degeneracy, but we already tested for that.
                if (Math.Abs(volume) < MathHelper.BigEpsilon)
                {
                    throw new ArgumentException("Point set is degenerate; convex hulls must have volume.");
                }
                if (volume < 0)
        ...
        private static void RemoveInsidePoints(RawList<Vector3f> points, RawList<int> triangleIndices, RawList<int> outsidePoints)
        {
            ...
            for (int i = 0; i < triangleIndices.Count && insidePoints.Count > 0; i += 3)
            {
                ...
                Vector3f p = points.Elements[triangleIndices.Elements[i]];
                ...
                for (int j = insidePoints.Count - 1; j >= 0; --j)
                {
                    ...
                    Vector3f offset;
                    Vector3f.Subtract(ref points.Elements[insidePoints.Elements[j]], ref p, out offset);
                    float dot;
                    Vector3f.Dot(ref offset, ref normal, out dot);
        ...
        private static void FindNormal(RawList<int> indices, RawList<Vector3f> points, int triangleIndex, out Vector3f normal)
        {
            var a = points.Elements[indices.Elements[triangleIndex]];
            Vector3f ab, ac;
            Vector3f.Subtract(ref points.Elements[indices.Elements[triangleIndex + 1]], ref a, out ab);
            Vector3f.Subtract(ref points.Elements[indices.Elements[triangleIndex + 2]], ref a, out ac);
            Vector3f.Cross(ref ac, ref ab, out normal);
        }

        private static bool IsTriangleVisibleFromPoint(RawList<int> indices, RawList<Vector3f> points, int triangleIndex, ref Vector3f point)
        {
            ...
            Vector3f offset;
            Vector3f.Subtract(ref point, ref a, out offset);
            float dot;
            Vector3f.Dot(ref offset, ref normal, out dot);
            return dot >= 0;
        }

        private static void VerifyWindings(RawList<int> newIndices, RawList<Vector3f> points, ref Vector3f centroid)
        {
