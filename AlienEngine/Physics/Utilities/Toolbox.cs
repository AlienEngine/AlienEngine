...
namespace AlienEngine.Core.Physics.Utilities
{
    ...
    public static class Toolbox
    {
        ...
        /// </summary>
        public static Vector3f BackVector = VectorHelper.Backward;

        /// <summary>
        /// Reference for a vector with dimensions (0,-1,0).
        /// </summary>
        public static Vector3f DownVector = VectorHelper.Down;

        /// <summary>
        /// Reference for a vector with dimensions (0,0,-1).
        /// </summary>
        public static Vector3f ForwardVector = VectorHelper.Forward;

        /// <summary>
        /// Reference for a vector with dimensions (-1,0,0).
        /// </summary>
        public static Vector3f LeftVector = VectorHelper.Left;

        /// <summary>
        /// Reference for a vector with dimensions (1,0,0).
        /// </summary>
        public static Vector3f RightVector = VectorHelper.Right;

        /// <summary>
        /// Reference for a vector with dimensions (0,1,0).
        /// </summary>
        public static Vector3f UpVector = VectorHelper.Up;
        ...
        public static bool FindRayTriangleIntersection(ref Ray ray, float maximumLength, ref Vector3f a, ref Vector3f b, ref Vector3f c, out bool hitClockwise, out RayHit hit)
        {
            hitClockwise = false;
            hit = new RayHit();
            Vector3f ab, ac;
        ...
        /// <param name="hit">Hit data of the ray, if any</param>
        /// <returns>Whether or not the ray and triangle intersect.</returns>
        public static bool FindRayTriangleIntersection(ref Ray ray, float maximumLength, TriangleSidedness sidedness, ref Vector3f a, ref Vector3f b, ref Vector3f c, out RayHit hit)
        {
            hit = new RayHit();
            Vector3f ab, ac;
            Vector3f.Subtract(ref b, ref a, out ab);
        ...
        public static bool GetSegmentPlaneIntersection(Vector3f a, Vector3f b, Vector3f d, Vector3f e, Vector3f f, out Vector3f q)
        {
            Plane p;
            p.Normal = Vector3f.Cross(e - d, f - d);
            p.D = Vector3f.Dot(p.Normal, d);
            float t;
        ...
        /// <param name="q">Intersection point.</param>
        /// <returns>Whether or not the segment intersects the plane.</returns>
        public static bool GetSegmentPlaneIntersection(Vector3f a, Vector3f b, Plane p, out Vector3f q)
        {
            float t;
        ...
        /// <param name="q">Intersection point.</param>
        /// <returns>Whether or not the segment intersects the plane.</returns>
        public static bool GetSegmentPlaneIntersection(Vector3f a, Vector3f b, Plane p, out float t, out Vector3f q)
        {
            return GetLinePlaneIntersection(ref a, ref b, ref p, out t, out q) && t >= 0 && t <= 1;
        }
        ...
        public static bool GetLinePlaneIntersection(ref Vector3f a, ref Vector3f b, ref Plane p, out float t, out Vector3f q)
        {
            Vector3f ab;
            Vector3f.Subtract(ref b, ref a, out ab);
            float denominator;
        ...
        /// <param name="q">Intersection point.</param>
        /// <returns>Whether or not the line intersects the plane.  If false, the line is parallel to the plane's surface.</returns>
        public static bool GetRayPlaneIntersection(ref Ray ray, ref Plane p, out float t, out Vector3f q)
        {
            float denominator;
        ...
        public static VoronoiRegion GetClosestPointOnTriangleToPoint(ref Vector3f a, ref Vector3f b, ref Vector3f c, ref Vector3f p, out Vector3f closestPoint)
        {
            float v, w;
            Vector3f ab;
            Vector3f.Subtract(ref b, ref a, out ab);
            ...
            {
                v = d1 / (d1 - d3);
                Vector3f.Multiply(ref ab, v, out closestPoint);
                Vector3f.Add(ref closestPoint, ref a, out closestPoint);
            ...
            float d5;
            Vector3f.Dot(ref ab, ref cp, out d5);
            float d6;
            Vector3f.Dot(ref ac, ref cp, out d6);
            if (d6 >= 0 && d5 <= d6)
            {
                closestPoint = c;
            ...
            {
                w = d2 / (d2 - d6);
                Vector3f.Multiply(ref ac, w, out closestPoint);
                Vector3f.Add(ref closestPoint, ref a, out closestPoint);
            ...
            {
                w = (d4 - d3) / ((d4 - d3) + (d5 - d6));
                Vector3f.Subtract(ref c, ref b, out closestPoint);
                Vector3f.Multiply(ref closestPoint, w, out closestPoint);
                Vector3f.Add(ref closestPoint, ref b, out closestPoint);
            ...
            w = vc * denom;
            Vector3f abv;
            Vector3f.Multiply(ref ab, v, out abv);
            Vector3f acw;
            Vector3f.Multiply(ref ac, w, out acw);
            Vector3f.Add(ref a, ref abv, out closestPoint);
            Vector3f.Add(ref closestPoint, ref acw, out closestPoint);
        ...
        public static void GetClosestPointOnTriangleToPoint(ref Vector3f a, ref Vector3f b, ref Vector3f c, ref Vector3f p, RawList<Vector3f> subsimplex, out Vector3f closestPoint)
        {
            subsimplex.Clear();
            float v, w;
            Vector3f ab;
        ...
        /// <param name="closestPoint">Closest point on tetrahedron to point.</param>
        [Obsolete("Used for simplex tests; consider using the PairSimplex and its variants instead for simplex-related testing.")]
        public static void GetClosestPointOnTriangleToPoint(RawList<Vector3f> q, int i, int j, int k, ref Vector3f p, RawList<int> subsimplex, RawList<float> baryCoords, out Vector3f closestPoint)
        {
            subsimplex.Clear();
            baryCoords.Clear();
            float v, w;
        ...
        public static void GetClosestPointOnSegmentToPoint(ref Vector3f a, ref Vector3f b, ref Vector3f p, out Vector3f closestPoint)
        {
            Vector3f ab;
            Vector3f.Subtract(ref b, ref a, out ab);
            Vector3f ap;
            Vector3f.Subtract(ref p, ref a, out ap);
            float t;
            Vector3f.Dot(ref ap, ref ab, out t);
            if (t <= 0)
            {
                closestPoint = a;
            }
            else
            {
                ...
                else
                {
                    t = t / denom;
                    Vector3f tab;
                    Vector3f.Multiply(ref ab, t, out tab);
                    Vector3f.Add(ref a, ref tab, out closestPoint);
                }
            }
        }
        ...
        /// <param name="closestPoint">Closest point on the edge to p.</param>
        [Obsolete("Used for simplex tests; consider using the PairSimplex and its variants instead for simplex-related testing.")]
        public static void GetClosestPointOnSegmentToPoint(ref Vector3f a, ref Vector3f b, ref Vector3f p, List<Vector3f> subsimplex, out Vector3f closestPoint)
        {
            subsimplex.Clear();
            Vector3f ab;
            Vector3f.Subtract(ref b, ref a, out ab);
        ...
        /// <param name="closestPoint">Closest point on the edge to p.</param>
        [Obsolete("Used for simplex tests; consider using the PairSimplex and its variants instead for simplex-related testing.")]
        public static void GetClosestPointOnSegmentToPoint(List<Vector3f> q, int i, int j, ref Vector3f p, List<int> subsimplex, List<float> baryCoords, out Vector3f closestPoint)
        {
            Vector3f a = q[i];
            Vector3f b = q[j];
            subsimplex.Clear();
        ...
        public static float GetSquaredDistanceFromPointToLine(ref Vector3f p, ref Vector3f a, ref Vector3f b)
        {
            Vector3f ap, ab;
            Vector3f.Subtract(ref p, ref a, out ap);
            Vector3f.Subtract(ref b, ref a, out ab);
            float e;
            Vector3f.Dot(ref ap, ref ab, out e);
        ...
        public static void GetClosestPointsBetweenSegments(Vector3f p1, Vector3f q1, Vector3f p2, Vector3f q2, out Vector3f c1, out Vector3f c2)
        {
            float s, t;
        ...
        /// <param name="c1">Closest point on first segment.</param>
        /// <param name="c2">Closest point on second segment.</param>
        public static void GetClosestPointsBetweenSegments(ref Vector3f p1, ref Vector3f q1, ref Vector3f p2, ref Vector3f q2,
                                                           out float s, out float t, out Vector3f c1, out Vector3f c2)
        {
        ...
        /// <param name="c1">Closest point on first segment.</param>
        /// <param name="c2">Closest point on second segment.</param>
        public static void GetClosestPointsBetweenLines(ref Vector3f p1, ref Vector3f q1, ref Vector3f p2, ref Vector3f q2,
                                                           out float s, out float t, out Vector3f c1, out Vector3f c2)
        {
            //Segment direction vectors
            Vector3f d1;
        ...
        public static bool ArePointsOnOppositeSidesOfPlane(ref Vector3f o, ref Vector3f p, ref Vector3f a, ref Vector3f b, ref Vector3f c)
        {
            Vector3f ab, ac, ap, ao;
            Vector3f.Subtract(ref b, ref a, out ab);
            Vector3f.Subtract(ref c, ref a, out ac);
            Vector3f.Subtract(ref p, ref a, out ap);
            Vector3f.Subtract(ref o, ref a, out ao);
            Vector3f q;
            Vector3f.Cross(ref ab, ref ac, out q);
            float signp;
            Vector3f.Dot(ref ap, ref q, out signp);
            float signo;
            Vector3f.Dot(ref ao, ref q, out signo);
            if (signp * signo <= 0)
                return true;
            return false;
        }
        ...
        public static float GetDistancePointToPlane(ref Vector3f point, ref Vector3f normal, ref Vector3f pointOnPlane)
        {
            Vector3f offset;
            Vector3f.Subtract(ref point, ref pointOnPlane, out offset);
            float dot;
            Vector3f.Dot(ref normal, ref offset, out dot);
        ...
        public static void GetPointProjectedOnPlane(ref Vector3f point, ref Vector3f normal, ref Vector3f pointOnPlane, out Vector3f projectedPoint)
        {
            float dot;
            Vector3f.Dot(ref normal, ref point, out dot);
            float dot2;
            Vector3f.Dot(ref pointOnPlane, ref normal, out dot2);
            float t = (dot - dot2) / normal.LengthSquared;
            Vector3f multiply;
            Vector3f.Multiply(ref normal, t, out multiply);
            Vector3f.Subtract(ref point, ref multiply, out projectedPoint);
        }
        ...
        public static bool IsPointWithinFaceExtrusion(Vector3f point, List<Plane> planes, Vector3f centroid)
        {
            foreach (Plane plane in planes)
            {
                ...
                if (!((centroidPlaneDot <= MathHelper.Epsilon && pointPlaneDot <= MathHelper.Epsilon) || (centroidPlaneDot >= -MathHelper.Epsilon && pointPlaneDot >= -MathHelper.Epsilon)))
                {
        ...
        public static void GetClosestPointOnTetrahedronToPoint(ref Vector3f a, ref Vector3f b, ref Vector3f c, ref Vector3f d, ref Vector3f p, out Vector3f closestPoint)
        {
            ...
            if (ArePointsOnOppositeSidesOfPlane(ref p, ref d, ref a, ref b, ref c))
            {
                ...
                Vector3f.Subtract(ref q, ref p, out pq);
        ...
        public static void GetClosestPointOnTetrahedronToPoint(ref Vector3f a, ref Vector3f b, ref Vector3f c, ref Vector3f d, ref Vector3f p, RawList<Vector3f> subsimplex, out Vector3f closestPoint)
        {
            // Start out assuming point inside all halfspaces, so closest to itself
            subsimplex.Clear();
        ...
        /// <param name="closestPoint">Closest point on the tetrahedron to the point.</param>
        [Obsolete("This method was used for older GJK simplex tests.  If you need simplex tests, consider the PairSimplex class and its variants.")]
        public static void GetClosestPointOnTetrahedronToPoint(RawList<Vector3f> tetrahedron, ref Vector3f p, RawList<int> subsimplex, RawList<float> baryCoords, out Vector3f closestPoint)
        {
            var subsimplexCandidate = CommonResources.GetIntList();
            var baryCoordsCandidate = CommonResources.GetFloatList();
            Vector3f a = tetrahedron[0];
            Vector3f b = tetrahedron[1];
            Vector3f c = tetrahedron[2];
            Vector3f d = tetrahedron[3];
            closestPoint = p;
            Vector3f pq;
            float bestSqDist = float.MaxValue;
            subsimplex.Clear();
            ...
            if (!baryCoordsFound)
            {
                ...
                float abcd = (new Matrix4f(tetrahedron[0].X, tetrahedron[0].Y, tetrahedron[0].Z, 1,
                ...
                float pbcd = (new Matrix4f(p.X, p.Y, p.Z, 1,
                    tetrahedron[1].X, tetrahedron[1].Y, tetrahedron[1].Z, 1,
                    tetrahedron[2].X, tetrahedron[2].Y, tetrahedron[2].Z, 1,
                    tetrahedron[3].X, tetrahedron[3].Y, tetrahedron[3].Z, 1)).Determinant;
                float apcd = (new Matrix4f(tetrahedron[0].X, tetrahedron[0].Y, tetrahedron[0].Z, 1,
                    p.X, p.Y, p.Z, 1,
                    tetrahedron[2].X, tetrahedron[2].Y, tetrahedron[2].Z, 1,
                    tetrahedron[3].X, tetrahedron[3].Y, tetrahedron[3].Z, 1)).Determinant;
                float abpd = (new Matrix4f(tetrahedron[0].X, tetrahedron[0].Y, tetrahedron[0].Z, 1,
                    tetrahedron[1].X, tetrahedron[1].Y, tetrahedron[1].Z, 1,
                    p.X, p.Y, p.Z, 1,
                    tetrahedron[3].X, tetrahedron[3].Y, tetrahedron[3].Z, 1)).Determinant;
                abcd = 1 / abcd;
        ...
        public static bool RayCastSphere(ref Ray ray, ref Vector3f spherePosition, float radius, float maximumLength, out RayHit hit)
        {
        ...
        public static void ExpandBoundingBox(ref BoundingBox boundingBox, ref Vector3f sweep)
        {
            if (sweep.X > 0)
        ...
        public static void GetTriangleBoundingBox(ref Vector3f a, ref Vector3f b, ref Vector3f c, out BoundingBox aabb)
        {
#if !WINDOWS
        ...
        public static void UpdateOrientationRK4(ref Quaternion q, ref Matrix3f localInertiaTensorInverse, ref Vector3f angularMomentum, float dt, out Quaternion newOrientation)
        {
        ...
        public static void DifferentiateQuaternion(ref Quaternion orientation, ref Matrix3f localInertiaTensorInverse, ref Vector3f angularMomentum, out Quaternion orientationChange)
        {
            Quaternion normalizedOrientation;
            Quaternion.Normalize(ref orientation, out normalizedOrientation);
            Matrix3f tempRotMat;
            Matrix3f.FromQuaternion(ref normalizedOrientation, out tempRotMat);
            Matrix3f tempInertiaTensorInverse;
            Matrix3f.MultiplyTransposed(ref tempRotMat, ref localInertiaTensorInverse, out tempInertiaTensorInverse);
            Matrix3f.Multiply(ref tempInertiaTensorInverse, ref tempRotMat, out tempInertiaTensorInverse);
            Vector3f halfspin;
            Vector3f.Transform(ref angularMomentum, ref tempInertiaTensorInverse, out halfspin);
            Vector3f.Multiply(ref halfspin, .5f, out halfspin);
            var halfspinQuaternion = new Quaternion(0, halfspin.X, halfspin.Y, halfspin.Z);
        ...
        public static void GetBarycentricCoordinates(ref Vector3f p, ref Vector3f a, ref Vector3f b, ref Vector3f c, out float aWeight, out float bWeight, out float cWeight)
        {
            Vector3f ab, ac;
            Vector3f.Subtract(ref b, ref a, out ab);
            Vector3f.Subtract(ref c, ref a, out ac);
            ...
            else
            {
                ...
                Vector3f.DistanceSquared(ref p, ref a, out distance1);
                Vector3f.DistanceSquared(ref p, ref b, out distance2);
                Vector3f.DistanceSquared(ref p, ref c, out distance3);
