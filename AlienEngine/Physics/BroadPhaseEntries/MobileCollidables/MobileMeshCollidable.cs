...
namespace AlienEngine.Core.Physics.BroadPhaseEntries.MobileCollidables
{
    ...
    public class MobileMeshCollidable : EntityCollidable
    {
        ...
        public override bool RayCast(Ray ray, float maximumLength, out RayHit rayHit)
        {
            ...
            Matrix3f.FromQuaternion(ref worldTransform.Orientation, out orientation);
            Vector3f.TransformTranspose(ref ray.Direction, ref orientation, out localRay.Direction);
            Vector3f.Subtract(ref ray.Position, ref worldTransform.Position, out localRay.Position);
            Vector3f.TransformTranspose(ref localRay.Position, ref orientation, out localRay.Position);
            ...
            if (Shape.solidity == MobileMeshSolidity.Solid)
            {
                ...
                {
                    //It was inside!
                    rayHit = new RayHit() { Location = ray.Position, Normal = Vector3f.Zero, T = 0 };
                    return true;
                ...
                else
                {
                    if (rayHit.T < maximumLength)
                    {
                        ...
                        Vector3f.Multiply(ref ray.Direction, rayHit.T, out rayHit.Location);
                        Vector3f.Add(ref rayHit.Location, ref ray.Position, out rayHit.Location);
                        Vector3f.Transform(ref rayHit.Normal, ref orientation, out rayHit.Normal);
                    }
                    else
                    {
        ...
        public override bool ConvexCast(ConvexShape castShape, ref RigidTransform startingTransform, ref Vector3f sweep, out RayHit hit)
        {
            if (Shape.solidity == MobileMeshSolidity.Solid)
            {
                ...
                if (Shape.IsLocalRayOriginInMesh(ref ray, out hit))
                {
                    ...
                    hit = new RayHit() { Location = startingTransform.Position, Normal = new Vector3f(), T = 0 };
                    return true;
                }
            }
            hit = new RayHit();
            ...
            Matrix3f.FromQuaternion(ref worldTransform.Orientation, out transform.LinearTransform);
