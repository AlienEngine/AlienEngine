...
namespace AlienEngine.Core.Physics.BroadPhaseEntries.MobileCollidables
{
    /// <summary>
    ...
    public static class CompoundHelper
    {
        ...
        static void SplitReposition(Entity a, Entity b, ref ShapeDistributionInformation distributionInfoA, ref ShapeDistributionInformation distributionInfoB, float weightA, float weightB)
        {
            ...
            Vector3f weightedA, weightedB;
            Vector3f.Multiply(ref distributionInfoA.Center, weightA, out weightedA);
            Vector3f.Multiply(ref distributionInfoB.Center, weightB, out weightedB);
            Vector3f newLocalCenter;
            Vector3f.Add(ref weightedA, ref weightedB, out newLocalCenter);
            Vector3f.Divide(ref newLocalCenter, weightA + weightB, out newLocalCenter);

            Vector3f localOffsetA;
            Vector3f localOffsetB;
            Vector3f.Subtract(ref distributionInfoA.Center, ref newLocalCenter, out localOffsetA);
            Vector3f.Subtract(ref distributionInfoB.Center, ref newLocalCenter, out localOffsetB);

            Vector3f originalPosition = a.position;

            b.Orientation = a.Orientation;
            Vector3f offsetA = Vector3f.Transform(localOffsetA, a.Orientation);
            Vector3f offsetB = Vector3f.Transform(localOffsetB, a.Orientation);
            a.Position = originalPosition + offsetA;
            b.Position = originalPosition + offsetB;

            Vector3f originalLinearVelocity = a.linearVelocity;
            Vector3f originalAngularVelocity = a.angularVelocity;
            a.AngularVelocity = originalAngularVelocity;
            b.AngularVelocity = originalAngularVelocity;
            a.LinearVelocity = originalLinearVelocity + Vector3f.Cross(originalAngularVelocity, offsetA);
            b.LinearVelocity = originalLinearVelocity + Vector3f.Cross(originalAngularVelocity, offsetB);
        }
            ...
            out float weightA, out float weightB)
        {
            ...
            for (int i = a.children.Count - 1; i >= 0; i--)
            {
                ...
                Vector3f weightedCenter;
                Vector3f.Multiply(ref entry.LocalTransform.Position, entry.Weight, out weightedCenter);
                Vector3f.Add(ref weightedCenter, ref distributionInfoA.Center, out distributionInfoA.Center);
            ...
            for (int i = b.children.Count - 1; i >= 0; i--)
            {
                ...
                Vector3f weightedCenter;
                Vector3f.Multiply(ref entry.LocalTransform.Position, entry.Weight, out weightedCenter);
                Vector3f.Add(ref weightedCenter, ref distributionInfoB.Center, out distributionInfoB.Center);
            ...
            if (weightA > 0)
                Vector3f.Divide(ref distributionInfoA.Center, weightA, out distributionInfoA.Center);

            if (weightB > 0)
                Vector3f.Divide(ref distributionInfoB.Center, weightB, out distributionInfoB.Center);
            ...
            Vector3f offsetA;
            Vector3f.Negate(ref distributionInfoA.Center, out offsetA);
            Vector3f offsetB;
            Vector3f.Negate(ref distributionInfoB.Center, out offsetB);
            ...
            for (int i = a.children.Count - 1; i >= 0; i--)
            {
                var child = a.children.Elements[i];
                var entry = child.Entry;
                Vector3f transformedOffset;
                Quaternion conjugate;
                Quaternion.Conjugate(ref entry.LocalTransform.Orientation, out conjugate);
                Vector3f.Transform(ref offsetA, ref conjugate, out transformedOffset);
                ...
                Matrix3f.Add(ref contribution, ref distributionInfoA.VolumeDistribution, out distributionInfoA.VolumeDistribution);
            }
            for (int i = b.children.Count - 1; i >= 0; i--)
            {
                var child = b.children.Elements[i];
                ...
                Quaternion conjugate;
                Quaternion.Conjugate(ref entry.LocalTransform.Orientation, out conjugate);
                Vector3f.Transform(ref offsetB, ref conjugate, out transformedOffset);
                child.CollisionInformation.localPosition = transformedOffset;
                Matrix3f contribution;
                ...
                Matrix3f.Add(ref contribution, ref distributionInfoB.VolumeDistribution, out distributionInfoB.VolumeDistribution);
            }

            //Normalize the volume distribution.
            Matrix3f.Multiply(ref distributionInfoA.VolumeDistribution, 1 / weightA, out distributionInfoA.VolumeDistribution);
            Matrix3f.Multiply(ref distributionInfoB.VolumeDistribution, 1 / weightB, out distributionInfoB.VolumeDistribution);
        ...
        static void RemoveReposition(Entity compound, ref ShapeDistributionInformation distributionInfo, float weight, float removedWeight, ref Vector3f removedCenter)
        {
            //The compounds are not aligned with the original's position yet.
            //In order to align them, first look at the centers the split method computed.
            //They are offsets from the center of the original shape in local space.
            //These can be used to reposition the objects in world space.
            Vector3f weightedA, weightedB;
            Vector3f.Multiply(ref distributionInfo.Center, weight, out weightedA);
            Vector3f.Multiply(ref removedCenter, removedWeight, out weightedB);
            Vector3f newLocalCenter;
            Vector3f.Add(ref weightedA, ref weightedB, out newLocalCenter);
            Vector3f.Divide(ref newLocalCenter, weight + removedWeight, out newLocalCenter);

            Vector3f localOffset;
            Vector3f.Subtract(ref distributionInfo.Center, ref newLocalCenter, out localOffset);

            Vector3f originalPosition = compound.position;

            Vector3f offset = Vector3f.Transform(localOffset, compound.orientation);
            compound.Position = originalPosition + offset;

            Vector3f originalLinearVelocity = compound.linearVelocity;
            Vector3f originalAngularVelocity = compound.angularVelocity;
            compound.AngularVelocity = originalAngularVelocity;
            compound.LinearVelocity = originalLinearVelocity + Vector3f.Cross(originalAngularVelocity, offset);
        }
        ...
        public static bool RemoveChildFromCompound(CompoundCollidable compound, Func<CompoundChild, bool> removalPredicate, IList<ShapeDistributionInformation> childContributions,
           out ShapeDistributionInformation distributionInfo, out float weight, out float removedWeight, out Vector3f removedCenter)
        {
            bool removalOccurred = false;
            removedWeight = 0;
            removedCenter = new Vector3f();
            for (int i = compound.children.Count - 1; i >= 0; i--)
            {
                ...
                if (removalPredicate(child))
                {
                    ...
                    Vector3f toAdd;
                    Vector3f.Multiply(ref entry.LocalTransform.Position, entry.Weight, out toAdd);
                    Vector3f.Add(ref removedCenter, ref toAdd, out removedCenter);
            ...
            if (removedWeight > 0)
            {
                Vector3f.Divide(ref removedCenter, removedWeight, out removedCenter);
            }
            ...
            for (int i = compound.children.Count - 1; i >= 0; i--)
            {
                ...
                Vector3f.Add(ref contribution.Center, ref entry.LocalTransform.Position, out contribution.Center);
                Vector3f.Multiply(ref contribution.Center, child.Entry.Weight, out contribution.Center);
                Vector3f.Add(ref contribution.Center, ref distributionInfo.Center, out distributionInfo.Center);
                distributionInfo.Volume += contribution.Volume;
                weight += entry.Weight;
            }
            //Average the center out.
            Vector3f.Divide(ref distributionInfo.Center, weight, out distributionInfo.Center);
            ...
            Vector3f offset;
            Vector3f.Negate(ref distributionInfo.Center, out offset);
            ...
            for (int i = compound.children.Count - 1; i >= 0; i--)
            {
                ...
                Quaternion conjugate;
                Quaternion.Conjugate(ref entry.LocalTransform.Orientation, out conjugate);
                Vector3f.Transform(ref offset, ref conjugate, out transformedOffset);
                child.CollisionInformation.localPosition = transformedOffset;
                var contribution = childContributions[child.shapeIndex];
                ...
                //Vector3f.Add(ref entry.LocalTransform.Position, ref offsetA, out entry.LocalTransform.Position);
                Matrix3f.Add(ref contribution.VolumeDistribution, ref distributionInfo.VolumeDistribution, out distributionInfo.VolumeDistribution);
            }

            //Normalize the volume distribution.
            Matrix3f.Multiply(ref distributionInfo.VolumeDistribution, 1 / weight, out distributionInfo.VolumeDistribution);
        ...
        public static CompoundCollidable CreatePartialCompoundCollidable(CompoundShape shape, IList<int> childIndices)
        {
            ...
            for (int i = 0; i < childIndices.Count; i++)
            {
                ...
                Vector3f toAdd;
                Vector3f.Multiply(ref entry.LocalTransform.Position, entry.Weight, out toAdd);
                Vector3f.Add(ref center, ref toAdd, out center);
            ...
            Vector3f.Divide(ref center, totalWeight, out center);
