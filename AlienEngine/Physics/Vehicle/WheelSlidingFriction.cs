...
namespace AlienEngine.Core.Physics.Vehicle
{
    ...
    public class WheelSlidingFriction : ISolverSettings
    {
        ...
        internal Vector3f slidingFrictionAxis;
        ...
        /// </summary>
        public Vector3f SlidingFrictionAxis
        {
        ...
        internal void PreStep(float dt)
        {
            ...
            Vector3f.Cross(ref wheel.worldForwardDirection, ref wheel.normal, out slidingFrictionAxis);
            float axisLength = slidingFrictionAxis.LengthSquared;
            //Safety against bad cross product
            if (axisLength < MathHelper.BigEpsilon)
            {
                Vector3f.Cross(ref wheel.worldForwardDirection, ref Toolbox.UpVector, out slidingFrictionAxis);
                axisLength = slidingFrictionAxis.LengthSquared;
                if (axisLength < MathHelper.BigEpsilon)
                {
                    Vector3f.Cross(ref wheel.worldForwardDirection, ref Toolbox.RightVector, out slidingFrictionAxis);
                }
            }
