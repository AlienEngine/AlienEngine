...
using AlienEngine.Core.Physics.BroadPhaseSystems;
...
namespace AlienEngine.Core.Physics.UpdateableSystems
{
    ...
    public class FluidVolume : Updateable, IDuringForcesUpdateable, ICollisionRulesOwner
    {
        ...
        private Matrix3f toSurfaceRotationMatrix;
        Vector3f upVector;
        ///<summary>
        ...
        private Vector3f flowDirection;
        /// <summary>
        ...
        private List<Vector3f[]> surfaceTriangles;
        /// <summary>
        /// List of coplanar triangles composing the surface of the fluid.
        /// </summary>
        public List<Vector3f[]> SurfaceTriangles
        {
            get
            {
        ...
        /// <param name="surfaceTriangles">List of triangles composing the surface of the fluid.  Set up as a list of length 3 arrays of Vector3f's.</param>
        ...
        public FluidVolume(Vector3f upVector, float gravity, List<Vector3f[]> surfaceTriangles, float depth, float fluidDensity, float linearDamping, float angularDamping)
        {
            Gravity = gravity;
        ...
        void GetSamplingOrigin(ref BoundingBox entityBoundingBox, out Vector3f xSpacing, out Vector3f zSpacing, out float perColumnArea, out Vector3f origin)
        {
            ...
            xSpacing = new Vector3f(widthIncrement, 0, 0);
            zSpacing = new Vector3f(0, 0, lengthIncrement);
            Vector3f.Transform(ref xSpacing, ref surfaceTransform.Orientation, out xSpacing);
            Vector3f.Transform(ref zSpacing, ref surfaceTransform.Orientation, out zSpacing);
            ...
            Vector3f offset;
            Vector3f.Multiply(ref xSpacing, .5f, out offset);
            Vector3f.Add(ref minimum, ref offset, out origin);
            Vector3f.Multiply(ref zSpacing, .5f, out offset);
            Vector3f.Add(ref origin, ref offset, out origin);
            ...
            //{
            //    Vector3f dir;
            //    Vector3f.Negate(ref upVector, out dir);
            ...
            //    Vector3f.Subtract(ref extremePoint, ref origin, out offset);
            //    float offsetX, offsetZ;
            //    Vector3f.Dot(ref offset, ref right, out offsetX);
            //    Vector3f.Dot(ref offset, ref backward, out offsetZ);
            //    offsetX %= widthIncrement;
            //    offsetZ %= lengthIncrement;

            //    if (offsetX > .5f * widthIncrement)
            //    {
            //        Vector3f.Multiply(ref right, 1 - offsetX, out offset);
            //    }
            //    else
            //    {
            //        Vector3f.Multiply(ref right, -offsetX, out offset);
            //    }

            //    if (offsetZ > .5f * lengthIncrement)
            //    {
            //        Vector3f temp;
            //        Vector3f.Multiply(ref right, 1 - offsetZ, out temp);
            //        Vector3f.Add(ref temp, ref offset, out offset);
            //    }
            //    else
            //    {
            //        Vector3f temp;
            //        Vector3f.Multiply(ref right, -offsetZ, out temp);
            //        Vector3f.Add(ref temp, ref offset, out offset);
            //    }
        ...
        float GetSubmergedHeight(EntityCollidable collidable, float maxLength, float boundingBoxHeight, ref Vector3f rayOrigin, ref Vector3f xSpacing, ref Vector3f zSpacing, int i, int j, out Vector3f volumeCenter)
        {
            Ray ray;
            Vector3f.Multiply(ref xSpacing, i, out ray.Position);
            Vector3f.Multiply(ref zSpacing, j, out ray.Direction);
            Vector3f.Add(ref ray.Position, ref ray.Direction, out ray.Position);
            Vector3f.Add(ref ray.Position, ref rayOrigin, out ray.Position);
            ...
            if (collidable.RayCast(ray, maxLength, out rayHit))
            {
                ...
                Vector3f.Multiply(ref ray.Direction, boundingBoxHeight, out ray.Direction);
                Vector3f.Add(ref ray.Position, ref ray.Direction, out ray.Position);
                Vector3f.Negate(ref upVector, out ray.Direction);
                ...
                Vector3f bottomPosition = rayHit.Location;
                if (collidable.RayCast(ray, boundingBoxHeight - rayHit.T, out rayHit))
                {
                    //Transform the hit into local space.
                    RigidTransform.TransformByInverse(ref rayHit.Location, ref surfaceTransform, out rayHit.Location);
                    Vector3f.Add(ref rayHit.Location, ref bottomPosition, out volumeCenter);
                    Vector3f.Multiply(ref volumeCenter, .5f, out volumeCenter);
