...
namespace AlienEngine.Core.Physics.Character
{
    ...
    public class CharacterContactCategorizer
    {
        ...
        public float TractionThreshold
        {
            ...
            set
            {
                if (value < MathHelper.BigEpsilon || value > 1)
                    throw new ArgumentException("Traction threshold values must range from " + MathHelper.BigEpsilon + " to 1, inclusive.");
        ...
        public float SupportThreshold
        {
            ...
            {
                if (value < MathHelper.BigEpsilon || value > 1)
                    throw new ArgumentException("Support threshold values must range from " + MathHelper.BigEpsilon + " to 1, inclusive.");
        ...
        public float HeadThreshold
        {
            ...
            set
            {
                if (value < -1 || value > -MathHelper.BigEpsilon)
                    throw new ArgumentException("Head threshold values must range from -1 to " + -MathHelper.BigEpsilon + ", inclusive.");
                headThreshold = value;
            }
        }
        ...
        public void CategorizeContacts<TOutputContacts>(CollidablePairHandler pair, EntityCollidable characterCollidable, ref Vector3f downDirection, 
                                                            ...
                                                            where TOutputContacts : IList<CharacterContact>
        {
            ...
            for (int i = pair.ContactCount - 1; i >= 0; --i)
            {
                ...
                float dot;
                Vector3f offset;
                Vector3f.Subtract(ref characterContact.Contact.Position, ref characterCollidable.worldTransform.Position, out offset);
                Vector3f.Dot(ref characterContact.Contact.Normal, ref offset, out dot);
                if (dot < 0)
                {
                    //The normal should face outward.
                    Vector3f.Negate(ref characterContact.Contact.Normal, out characterContact.Contact.Normal);
                }

                Vector3f.Dot(ref characterContact.Contact.Normal, ref downDirection, out dot);
