...
namespace AlienEngine.Core.Physics.Character
{
    ...
    public class StepManager
    {
        ...
        public bool TryToStepDown(out Vector3f newPosition)
        {
            ...
            Vector3f normal = SupportFinder.SupportRayData.Value.HitData.Normal;
            ...
            Plane plane = new Plane(normal, Vector3f.Dot(SupportFinder.SupportRayData.Value.HitData.Location, normal));
            Vector3f candidatePosition;
            ...
            try
            {
                ...
                while (attempts++ < 5 && lowestBound - highestBound > MathHelper.BigEpsilon)
                {
        ...
        CharacterContactPositionState TryDownStepPosition(ref Vector3f position, ref Vector3f down,
            ...
            out float hintOffset)
        {
            ...
            if (tractionContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    ...
                    hintOffset = -Vector3f.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth;
                ...
                else if (supportState == CharacterContactPositionState.TooDeep)
                {
                    ...
                    hintOffset = Math.Min(0, .001f - Vector3f.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth);
                ...
                else //if (supportState == SupportState.Separated)
                {
                    ...
                    hintOffset = -.001f - Vector3f.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth;
        ...
        public bool TryToStepUp(out Vector3f newPosition)
        {
        ...
        void FindUpStepCandidates(RawList<ContactData> outputStepCandidates, ref Vector3f down)
        {
            Vector3f movementDirection = HorizontalMotionConstraint.MovementDirection3d;
            foreach (var c in SupportFinder.SideContacts)
            {
                ...
                float dot;
                Vector3f.Dot(ref contact.Normal, ref movementDirection, out dot);
                if (dot > 0)
                {
                    //It is! But is it low enough?
                    dot = Vector3f.Dot(down, c.Contact.Position - characterBody.Position);
                    ...
                    if (dot < characterBody.Height * .5f && dot > characterBody.Height * .5f - maximumStepHeight - upStepMargin)
                    {
                        ...
                        {
                            Vector3f.Dot(ref outputStepCandidates.Elements[i].Normal, ref contact.Normal, out dot);
                            if (dot > .99f)
                            {
        ...
        bool TryToStepUsingContact(ref ContactData contact, ref Vector3f down, out Vector3f newPosition)
        {
            Vector3f position = characterBody.Position;
            ...
            float dot;
            Vector3f.Dot(ref normal, ref down, out dot);
            Vector3f error;
            Vector3f.Multiply(ref down, dot, out error);
            Vector3f.Subtract(ref normal, ref error, out normal);
            normal.Normalize();
            ...
            Vector3f.Multiply(ref down, characterBody.Height * .5f - downRayLength, out ray.Position);
            Vector3f.Add(ref ray.Position, ref position, out ray.Position);
            ...
            Vector3f horizontalOffset;
            Vector3f.Multiply(ref normal, length, out horizontalOffset);
            Vector3f.Add(ref ray.Position, ref horizontalOffset, out ray.Position);
            ray.Direction = down;
            ...
            Vector3f supportNormal;
            Vector3f.Normalize(ref earliestHit.Normal, out supportNormal);
            //Calibrate the normal to face in the same direction as the down vector for consistency.
            Vector3f.Dot(ref supportNormal, ref down, out dot);
            if (dot < 0)
            {
                Vector3f.Negate(ref supportNormal, out supportNormal);
                dot = -dot;
            }
            ...
            Vector3f.Negate(ref down, out ray.Direction);
            //Find the earliest hit, if any.
            ...
            Vector3f.Multiply(ref normal, horizontalOffsetAmount, out horizontalOffset);
            Vector3f.Add(ref transform.Position, ref horizontalOffset, out transform.Position);
            Vector3f verticalOffset;
            Vector3f.Multiply(ref down, -downRayLength, out verticalOffset);
            Vector3f.Add(ref transform.Position, ref verticalOffset, out transform.Position);
            ...
            //Intersect the ray against the plane defined by the support hit.
            Vector3f intersection;
            Vector3f.Dot(ref earliestHit.Location, ref supportNormal, out dot);
        ...
        private void PrepareQueryObject(ref Vector3f position)
        {
        ...
        CharacterContactPositionState TryUpStepPosition(ref Vector3f sideNormal, ref Vector3f position, ref Vector3f down,
            ref QuickList<CharacterContact> tractionContacts, ref QuickList<CharacterContact> supportContacts, ref QuickList<CharacterContact> sideContacts, ref QuickList<CharacterContact> headContacts,
            out float hintOffset)
        {
            hintOffset = 0;
            ...
            if (headContacts.Count > 0)
            {
                ...
                float dot;
                Vector3f.Dot(ref down, ref headContacts.Elements[0].Contact.Normal, out dot);
                hintOffset = -dot * headContacts.Elements[0].Contact.PenetrationDepth;
                for (int i = 1; i < headContacts.Count; i++)
                {
                    Vector3f.Dot(ref down, ref headContacts.Elements[i].Contact.Normal, out dot);
            ...
            if (!obstructed && supportContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    if (tractionContacts.Count > 0)
                    {
                        ...
                        hintOffset = Math.Min(0, Vector3f.Dot(supportContact.Contact.Normal, down) * (CollisionDetectionSettings.AllowedPenetration * .5f - supportContact.Contact.PenetrationDepth));
                        return CharacterContactPositionState.Accepted;
                    }
                    else
                    {
                        ...
                        float verticalOffset = Vector3f.Dot(downRay.Position - position, down);
                        ...
                        if (!QueryManager.RayCastHitAnything(obstructionTestRay, 1))
                        {
                            ...
                            if (QueryManager.RayCast(downRay, characterBody.Height, out hit))
                            {
                                ...
                                if (characterBody.Height - maximumStepHeight < hit.T)
                                {
                                    ...
                                    Vector3f.Dot(ref hit.Normal, ref down, out dot);
        ...
        bool IsUpStepObstructedBySideContacts(ref Vector3f sideNormal, ref QuickList<CharacterContact> sideContacts)
        {
            //A contact is considered obstructive if its projected depth is deeper than any existing contact along the existing contacts' normals.
            for (int i = 0; i < sideContacts.Count; i++)
            {
        ...
        bool IsObstructiveToUpStepping(ref Vector3f sideNormal, ref ContactData contact)
        {
            ...
            float dot;
            Vector3f.Dot(ref contact.Normal, ref sideNormal, out dot);
            ...
            foreach (var c in SupportFinder.SideContacts)
            {
                dot = Vector3f.Dot(contact.Normal, c.Contact.Normal);
