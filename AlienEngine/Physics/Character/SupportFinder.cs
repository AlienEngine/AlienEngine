...
namespace AlienEngine.Core.Physics.Character
{
    ...
    public class SupportFinder
    {
        ...
        private void UpdateSupportData(ref Vector3f down)
        {
            ...
            else
            {
                ...
                if (SupportRayData != null)
                {
                    supportData = new SupportData
                    {
                        ...
                        Depth = Vector3f.Dot(down, SupportRayData.Value.HitData.Normal) * (BottomDistance - SupportRayData.Value.HitData.T),
            ...
            {
                Vector3f.Add(ref supportData.Position, ref contacts.Elements[i].Contact.Position, out supportData.Position);
                Vector3f.Add(ref supportData.Normal, ref contacts.Elements[i].Contact.Normal, out supportData.Normal);
            }
            if (contacts.Count > 1)
            {
                Vector3f.Divide(ref supportData.Position, contacts.Count, out supportData.Position);
                ...
                {
                    Vector3f.Multiply(ref supportData.Normal, 1 / (float)Math.Sqrt(length), out supportData.Normal);
                }
            }
            ...
            {
                float dot;
                Vector3f.Dot(ref contacts.Elements[i].Contact.Normal, ref supportData.Normal, out dot);
        ...
        private void UpdateVerticalSupportData(ref Vector3f down, ref Vector3f movementDirection)
        {
            if (HasTraction)
            {
                if (tractionContacts.Count > 0)
                {
                    ...
                    {
                        float dot;
                        Vector3f.Dot(ref movementDirection, ref tractionContacts.Elements[i].Contact.Normal, out dot);
                        if (dot > greatestDot)
                        {
                            greatestDot = dot;
                            greatestIndex = i;
                    ...
                    {
                        float dot;
                        Vector3f.Dot(ref tractionContacts.Elements[i].Contact.Normal, ref verticalSupportData.Normal, out dot);
                ...
                verticalSupportData.Depth = Vector3f.Dot(down, SupportRayData.Value.HitData.Normal) * (BottomDistance - SupportRayData.Value.HitData.T);
        ...
        /// </summary>
        public void UpdateSupports(ref Vector3f movementDirection)
        {
            ...
            Vector3f bodyPosition = characterBody.position;
            ...
            if (!HasSupport && hadTraction)
            {
                ...
                if (!HasTraction && tryingToMove)
                {
                    ...
                    Vector3f.Cross(ref movementDirection, ref downDirection, out horizontalOffset);
                    Vector3f.Multiply(ref horizontalOffset, rayCastInnerRadius, out horizontalOffset);
                ...
                if (!HasTraction && tryingToMove)
                {
                    //Compute the horizontal offset direction.  Down direction and the movement direction are normalized and perpendicular, so the result is too.
                    Vector3f horizontalOffset;
                    Vector3f.Cross(ref downDirection, ref movementDirection, out horizontalOffset);
                    Vector3f.Multiply(ref horizontalOffset, rayCastInnerRadius, out horizontalOffset);
                    Ray ray = new Ray(bodyPosition + horizontalOffset, downDirection);
        ...
        bool TryDownCast(ref Ray ray, float length, out bool hasTraction, out SupportRayData supportRayData)
        {
            ...
            if (QueryManager.RayCast(ray, length, out earliestHit, out earliestHitObject))
            {
                ...
                Vector3f.Divide(ref earliestHit.Normal, (float)Math.Sqrt(lengthSquared), out earliestHit.Normal);
                //A collidable was hit!  It's a support, but does it provide traction?
                earliestHit.Normal.Normalize();
                float dot;
                Vector3f.Dot(ref ray.Direction, ref earliestHit.Normal, out dot);
                if (dot < 0)
                {
                    //Calibrate the normal so it always faces the same direction relative to the body.
                    Vector3f.Negate(ref earliestHit.Normal, out earliestHit.Normal);
                    dot = -dot;
                }
        ...
        internal bool IsSideContactObstructive(ref ContactData contact)
        {
            ...
            foreach (var c in SideContacts)
            {
                ...
                float dot = Vector3f.Dot(contact.Normal, c.Contact.Normal);
                float depth = dot * c.Contact.PenetrationDepth + MathHelper.BigEpsilon;
