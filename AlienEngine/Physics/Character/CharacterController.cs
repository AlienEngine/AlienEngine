    ...
    public class CharacterController : Updateable, IBeforeSolverUpdateable
    {
        ...
        public Vector3f Down
        {
            ...
            set
            {
                ...
                if (lengthSquared < MathHelper.Epsilon)
                    ...
                    Vector3f.Divide(ref value, (float)Math.Sqrt(lengthSquared), out value);
        ...
        public Vector3f ViewDirection
        {
            ...
            set
            {
                ...
                {
                    Vector3f.Divide(ref value, (float)Math.Sqrt(lengthSquared), out viewDirection);
                }
                else
                {
                    value = Vector3f.Cross(Down, Toolbox.UpVector);
                    ...
                    {
                        value = Vector3f.Cross(Down, Toolbox.ForwardVector);
                        Vector3f.Normalize(ref value, out viewDirection);
                    }
                }
            }
        }
            ...
            )
        {
            ...
            Body.LocalInertiaTensorInverse = new Matrix3f();
            ...
            try
            {
                ...
                if (TryToJump)
                {
                    ...
                    else if (SupportFinder.HasSupport)
                    {
                        ...
                        float currentNormalVelocity = Vector3f.Dot(supportData.Normal, relativeVelocity);
                        //Target velocity is JumpSpeed.
        ...
        SupportData TeleportToPosition(Vector3f newPosition, float dt)
        {
        ...
        void CorrectContacts()
        {
            ...
            Vector3f downDirection = Body.OrientationMatrix.Down;
            ...
            foreach (var pair in Body.CollisionInformation.Pairs)
            {
                foreach (var contactData in pair.Contacts)
                {
                    ...
                    Vector3f offset = contact.Position - Body.Position;
                    Vector3f.Dot(ref offset, ref downDirection, out dot);
                    if (dot > minimumHeight)
                    {
                        ...
                        Vector3f.Multiply(ref downDirection, dot, out horizontalOffset);
                        Vector3f.Subtract(ref offset, ref horizontalOffset, out horizontalOffset);
                        float length = horizontalOffset.LengthSquared;
                        if (length > coreRadiusSquared)
                        {
                            //It's beyond the edge of the cylinder; clamp it.
                            Vector3f.Multiply(ref horizontalOffset, coreRadius / (float)Math.Sqrt(length), out horizontalOffset);
                        }
                        //It's on the bottom, so add the bottom height.
                        Vector3f closestPointOnCylinder;
                        Vector3f.Multiply(ref downDirection, minimumHeight, out closestPointOnCylinder);
                        Vector3f.Add(ref closestPointOnCylinder, ref horizontalOffset, out closestPointOnCylinder);
                        Vector3f.Add(ref closestPointOnCylinder, ref position, out closestPointOnCylinder);

                        //Compute the offset from the cylinder to the offset.
                        Vector3f offsetDirection;
                        Vector3f.Subtract(ref contact.Position, ref closestPointOnCylinder, out offsetDirection);
                        length = offsetDirection.LengthSquared;
                        if (length > MathHelper.Epsilon)
                        {
                            //Normalize the offset.
                            Vector3f.Divide(ref offsetDirection, (float)Math.Sqrt(length), out offsetDirection);
                        }
                        else
                            continue; //If there's no offset, it's really deep and correcting this contact might be a bad idea.

                        Vector3f.Dot(ref offsetDirection, ref downDirection, out dot);
                        float dotOriginal;
                        Vector3f.Dot(ref contact.Normal, ref downDirection, out dotOriginal);
                        if (dot > Math.Abs(dotOriginal)) //if the new offsetDirection normal is less steep than the original slope...
                        {
                            //Then use it!
                            Vector3f.Dot(ref offsetDirection, ref contact.Normal, out dot);
                            if (dot < 0)
                            {
                                //Don't flip the normal relative to the contact normal.  That would be bad!
                                Vector3f.Negate(ref offsetDirection, out offsetDirection);
                                dot = -dot;
                            }
        ...
        void ComputeRelativeVelocity(ref SupportData supportData, out Vector3f relativeVelocity)
        {
            ...
            if (SupportFinder.HasSupport)
            {
                ...
                if (entityCollidable != null)
                {
                    ...
                    Vector3f.Subtract(ref relativeVelocity, ref entityVelocity, out relativeVelocity);
                }
            }
        ...
        void ApplyJumpVelocity(ref SupportData supportData, Vector3f velocityChange, ref Vector3f relativeVelocity)
        {
            ...
            if (entityCollidable != null)
            {
                ...
                {
                    Vector3f change = velocityChange * jumpForceFactor;
            ...
            Vector3f.Add(ref relativeVelocity, ref velocityChange, out relativeVelocity);
        ...
        public override void OnAdditionToSpace(Space newSpace)
        {
            ...
            Body.AngularVelocity = new Vector3f();
