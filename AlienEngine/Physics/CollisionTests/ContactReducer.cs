...
namespace AlienEngine.Core.Physics.CollisionTests
{
    ...
    public static class ContactReducer
    {
        ...
        public static void ReduceContacts(RawList<Contact> contacts, ref QuickList<ContactData> contactCandidates, RawList<int> contactsToRemove, ref QuickList<ContactData> toAdd)
        {
            ...
            {
                Vector3f.Add(ref normal, ref contacts.Elements[i].Normal, out normal);
            ...
            {
                Vector3f.Add(ref normal, ref contactCandidates.Elements[i].Normal, out normal);
            ...
            for (int i = 0; i < contacts.Count; i++)
            {
                Vector3f.DistanceSquared(ref contacts.Elements[i].Position, ref deepestPosition, out distanceSquared);
            ...
            for (int i = 0; i < contactCandidates.Count; i++)
            {
                Vector3f.DistanceSquared(ref contactCandidates.Elements[i].Position, ref deepestPosition, out distanceSquared);
                if (distanceSquared > furthestDistance)
                {
                    furthestDistance = distanceSquared;
            ...
            Vector3f xAxis;
            Vector3f.Subtract(ref deepestPosition, ref furthestPosition, out xAxis);

            //Create the second axis of the 2d 'coordinate system' of the manifold.
            Vector3f yAxis;
            Vector3f.Cross(ref xAxis, ref normal, out yAxis);
            ...
            {
                float dot;
                Vector3f.Dot(ref contacts.Elements[i].Position, ref yAxis, out dot);
                if (dot < minYAxisDot)
                {
                    minYAxisIndex = i;
                    minYAxisDot = dot;
            ...
            {
                float dot;
                Vector3f.Dot(ref contactCandidates.Elements[i].Position, ref yAxis, out dot);
                if (dot < minYAxisDot)
                {
        ...
        public static void ReduceContacts(RawList<Contact> contacts, ref ContactData contactCandidate, RawList<int> toRemove, out bool addCandidate)
        {
            ...
            Vector3f.DistanceSquared(ref contactCandidate.Position, ref deepestPosition, out distanceSquared);
            if (distanceSquared > furthestDistance)
            {
                furthestIndex = 4;
            }
            ...
            //Create the second axis of the 2d 'coordinate system' of the manifold.
            Vector3f yAxis;
            Vector3f.Cross(ref xAxis, ref contacts.Elements[0].Normal, out yAxis);
            ...
            Vector3f.Dot(ref contactCandidate.Position, ref yAxis, out dot);
            if (dot < minYAxisDot)
            {
                minYAxisIndex = 4;
            }
