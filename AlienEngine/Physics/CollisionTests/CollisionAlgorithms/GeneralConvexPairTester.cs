...
namespace AlienEngine.Core.Physics.CollisionTests.CollisionAlgorithms
{
    ...
    public class GeneralConvexPairTester
    {
        ...
        private bool DoShallowContact(out ContactData contact)
        {
            ...
            //Vector3f closestAnew, closestBnew;
            ...
            //Vector3f closestAold, closestBold;
            ...
            //    Vector3f.DistanceSquared(closestAnew, closestBnew) - Vector3f.DistanceSquared(closestA, closestB) > .0001f &&
            //    (Vector3f.DistanceSquared(closestA, closestAnew) > .0001f ||
            //    Vector3f.DistanceSquared(closestB, closestBnew) > .0001f)))// ||
            //    //Math.Abs(Vector3f.Dot(closestB - closestA, closestBnew - closestAnew) - Vector3f.Dot(closestB - closestA, closestB - closestA)) > Toolbox.Epsilon)))
            //    Debug.WriteLine("Break.");

            //Vector3f sub;
            //Vector3f.Subtract(ref closestA, ref closestB, out sub);
            ...
            if (distanceSquared < MathHelper.Epsilon)
            {
            ...
            if (distanceSquared < margin * margin)
            {
                ...
                if (margin > MathHelper.Epsilon) //Avoid a NaN!
                    Vector3f.Multiply(ref displacement, collidableA.Shape.collisionMargin / margin, out contact.Position); //t * AB
                else
        ...
        private bool DoDeepContact(out ContactData contact)
        {
            ...
            if (previousState == CollisionState.Separated) //If it was shallow before, then its closest points will be used to find the normal.
            {
                ...
                //Vector3f velocityA;
                //Vector3f.Cross(ref contact.Position, ref collidableA.entity.angularVelocity, out velocityA);
                //Vector3f.Add(ref velocityA, ref collidableA.entity.linearVelocity, out velocityA);
                //Vector3f velocityB;
                //Vector3f.Subtract(ref contact.Position, ref localTransformB.Position, out velocityB);
                //Vector3f.Cross(ref velocityB, ref collidableB.entity.angularVelocity, out velocityB);
                //Vector3f.Add(ref velocityB, ref collidableB.entity.linearVelocity, out velocityB);
                ////The velocity is negated because the direction so point backwards along the velocity.
                //Vector3f.Subtract(ref velocityA, ref velocityB, out localDirection);
                ...
                if (collidableA.entity != null && collidableB.entity != null)
                    Vector3f.Subtract(ref collidableA.entity.linearVelocity, ref collidableB.entity.linearVelocity, out localDirection);
                else
                    localDirection = localSeparatingAxis;

                if (localDirection.LengthSquared < MathHelper.Epsilon)
                {
                    localDirection = VectorHelper.Up;
                }
            ...
            //    {
            //        Vector3f.Divide(ref localDirection, (float)Math.Sqrt(lengthSquared), out rayCastDirection);// (Vector3f.Normalize(localDirection) + Vector3f.Normalize(collidableB.worldTransform.Position - collidableA.worldTransform.Position)) / 2;
            ...
            //    Vector3f.Add(ref contact.Position, ref collidableA.worldTransform.Position, out contact.Position);
            //    if (contact.PenetrationDepth < collidableA.Shape.collisionMargin + collidableB.Shape.collisionMargin)
            //        state = CollisionState.ShallowContact;
            //    return true;
            //}
            ...
            //if (MPRTesting.GetLocalOverlapPosition(collidableA.Shape, collidableB.Shape, ref localTransformB, out contact.Position))
            //{
            //    Vector3f rayCastDirection = localTransformB.Position;
            //    MPRTesting.LocalSurfaceCast(collidableA.Shape, collidableB.Shape, ref localTransformB, ref rayCastDirection, out contact.PenetrationDepth, out contact.Normal);
            //    MPRTesting.LocalSurfaceCast(collidableA.Shape, collidableB.Shape, ref localTransformB, ref contact.Normal, out contact.PenetrationDepth, out rayCastDirection);
            //    RigidTransform.Transform(ref contact.Position, ref collidableA.worldTransform, out contact.Position);
            //    Vector3f.Transform(ref contact.Normal, ref collidableA.worldTransform.Orientation, out contact.Normal);
            //    return true;
            //}
