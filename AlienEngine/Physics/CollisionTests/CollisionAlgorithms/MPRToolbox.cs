...
namespace AlienEngine.Core.Physics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class MPRToolbox
    {
        ...
        public static bool GetOverlapPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB, out Vector3f position)
        {
        ...
        /// <returns>Whether or not the two shapes overlap.</returns>
        public static bool GetLocalOverlapPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, out Vector3f position)
        {
            return GetLocalOverlapPosition(shapeA, shapeB, ref localTransformB.Position, ref localTransformB, out position);
        }

        internal static bool GetLocalOverlapPosition(ConvexShape shapeA, ConvexShape shapeB, ref Vector3f originRay, ref RigidTransform localTransformB, out Vector3f position)
        {
            ...
            if (originRay.LengthSquared < MathHelper.Epsilon)
            {
            ...
            Vector3f v0;
            Vector3f.Negate(ref originRay, out v0); //Since we're in A's local space, A-B is just -B.
            ...
            Vector3f n = originRay;
            Vector3f v1;
            Vector3f v1A, v1B; //extreme point contributions from each shape.  Used later to compute contact position; could be used to cache simplex too.
            ...
            Vector3f v2;
            Vector3f v2A, v2B;
            Vector3f.Cross(ref v1, ref v0, out n);
            if (n.LengthSquared < MathHelper.Epsilon)
            {
                ...
                float dot;
                Vector3f.Dot(ref v1, ref originRay, out dot);
                if (dot < 0)
                {
                    //Origin is outside.
                    position = new Vector3f();
                ...
                Vector3f.Dot(ref v0, ref originRay, out dotv0);
                float barycentricCoordinate = -dotv0 / (dot - dotv0);
                //Vector3f.Subtract(ref v1A, ref v0A, out offset); //'v0a' is just the zero vector, so there's no need to calculate the offset.
                Vector3f.Multiply(ref v1A, barycentricCoordinate, out position);
                return true;
            }
            ...
            Vector3f.Subtract(ref v1, ref v0, out temp1);
            Vector3f.Subtract(ref v2, ref v0, out temp2);
            Vector3f.Cross(ref temp1, ref temp2, out n);
            ...
            while (true)
            {
                ...
                Vector3f.Cross(ref v1, ref v3, out temp1);
                float dot;
                Vector3f.Dot(ref temp1, ref v0, out dot);
                if (dot < 0)
                {
                ...
                if (dot < 0)
                {
                    ...
                    v1B = v3B;
                    // Calculate the normal of the plane that will be used to find a new extreme point.
                    Vector3f.Subtract(ref v2, ref v0, out temp1);
                    Vector3f.Subtract(ref v3, ref v0, out temp2);
                    Vector3f.Cross(ref temp1, ref temp2, out n);
                    continue;
                }
            ...
            while (true)
            {
                ...
                Vector3f.Subtract(ref v3, ref v2, out temp1);
                Vector3f.Subtract(ref v1, ref v2, out temp2);
                Vector3f.Cross(ref temp1, ref temp2, out n);
                float dot;
                Vector3f.Dot(ref n, ref v1, out dot);
                if (dot >= 0)
                {
                    Vector3f temp3;
                    ...
                    Vector3f.Subtract(ref v1, ref v0, out temp1);
                    Vector3f.Subtract(ref v2, ref v0, out temp2);
                    Vector3f.Subtract(ref v3, ref v0, out temp3);

                    Vector3f cross;
                    Vector3f.Cross(ref temp1, ref temp2, out cross);
                    float v0v1v2v3volume;
                    Vector3f.Dot(ref cross, ref temp3, out v0v1v2v3volume);

                    Vector3f.Cross(ref v1, ref v2, out cross);
                    float ov1v2v3volume;
                    Vector3f.Dot(ref cross, ref v3, out ov1v2v3volume);

                    Vector3f.Cross(ref originRay, ref temp2, out cross);
                    float v0ov2v3volume;
                    Vector3f.Dot(ref cross, ref temp3, out v0ov2v3volume);

                    Vector3f.Cross(ref temp1, ref originRay, out cross);
                    float v0v1ov3volume;
                    Vector3f.Dot(ref cross, ref temp3, out v0v1ov3volume);

                    if (v0v1v2v3volume > MathHelper.Epsilon * .01f)
                    {
                ...
                float dot2;
                Vector3f.Dot(ref v4, ref n, out dot2);
                if (dot2 < 0)
                {
                ...
                Vector3f.Cross(ref v4, ref v0, out temp1);
                Vector3f.Dot(ref v1, ref temp1, out dot);
                if (dot >= 0)
                {
                    Vector3f.Dot(ref v2, ref temp1, out dot);
                    if (dot >= 0)
                    {
                ...
                {
                    Vector3f.Dot(ref v3, ref temp1, out dot);
                    if (dot >= 0)
                    {
        ...
        /// <param name="localTransformB">Relative transform of shape B to shape A.</param>
        /// <returns>Whether or not the shapes are overlapping.</returns>
        internal static bool AreLocalShapesOverlapping(ConvexShape shapeA, ConvexShape shapeB, ref Vector3f originRay, ref RigidTransform localTransformB)
        {
            //Compute the origin ray.  This points from a point known to be inside the minkowski sum to the origin.
            //The centers of the shapes are used to create the interior point.
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3f direction, out float t, out Vector3f normal)
        {
            ...
            //Find another extreme point in a direction perpendicular to the previous.
            Vector3f v2;
            Vector3f.Cross(ref direction, ref v1, out n);
            if (n.LengthSquared < MathHelper.Epsilon)
            {
                //v1 and v0 could be parallel.
                //This isn't a bad thing- it means the direction is exactly aligned with the extreme point offset.
                ...
                if (rayLengthSquared > MathHelper.Epsilon * .01f)
                    Vector3f.Divide(ref direction, (float)Math.Sqrt(rayLengthSquared), out normal);
                else
                    normal = new Vector3f();

                float rate;
                Vector3f.Dot(ref  normal, ref direction, out rate);
                float distance;
                Vector3f.Dot(ref  normal, ref v1, out distance);
                if (rate > 0)
                    t = distance / rate;
                else
                    t = 0;
            ...
            Vector3f temp1, temp2;
            //Set n for the first iteration.
            Vector3f.Cross(ref v1, ref v2, out n);
            ...
            while (true)
            {
                ...
                Vector3f.Cross(ref v1, ref v3, out temp1);
                Vector3f.Dot(ref temp1, ref direction, out dot);
                if (dot < 0)
                {
                    //Replace the point that was on the inside of the plane (v2) with the new extreme point.
                    v2 = v3;
                    // Calculate the normal of the plane that will be used to find a new extreme point.
                    Vector3f.Cross(ref v1, ref v3, out n);
                    continue;
                }
                ...
                if (dot < 0)
                {
                    ...
                    v1 = v3;
                    // Calculate the normal of the plane that will be used to find a new extreme point.
                    Vector3f.Cross(ref v2, ref v3, out n);
                    continue;
                }
                break;
            }
            ...
            {
                //Compute the outward facing normal.
                Vector3f.Subtract(ref v1, ref v2, out temp1);
                Vector3f.Subtract(ref v3, ref v2, out temp2);
                Vector3f.Cross(ref temp1, ref temp2, out n);
                ...
                Vector3f.Dot(ref n, ref v1, out dot);
                float supportDot;
                Vector3f.Dot(ref v4, ref n, out supportDot);
                ...
                if (supportDot - dot < surfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    //Vector3f.Multiply(ref normal, normalLengthInverse, out normal);
                    ////Find the distance from the origin to the plane.
                    //t = dot * normalLengthInverse;

                    float lengthSquared = n.LengthSquared;
                    if (lengthSquared > MathHelper.Epsilon * .01f)
                    {
                        Vector3f.Divide(ref n, (float)Math.Sqrt(lengthSquared), out normal);

                        //The plane is very close to the surface, and the ray is known to pass through it.
                        //dot is the rate.
                        Vector3f.Dot(ref normal, ref direction, out dot);
                        //supportDot is the distance to the plane.
                        Vector3f.Dot(ref normal, ref v1, out supportDot);
                        if (dot > 0)
                            t = supportDot / dot;
                        else
                            t = 0;
                ...
                Vector3f.Cross(ref v4, ref direction, out temp1);
                Vector3f.Dot(ref v1, ref temp1, out dot);
                if (dot >= 0)
                {
                    Vector3f.Dot(ref v2, ref temp1, out dot);
                ...
                //Vector3f.Cross(ref v1, ref v4, out temp1);
                //Vector3f.Dot(ref temp1, ref direction, out dot);
                //if (dot > 0)
                //{
                //    Vector3f.Cross(ref v2, ref v4, out temp2);
                //    Vector3f.Dot(ref temp2, ref direction, out dot);
                //    if (dot > 0)
                //    {
                ...
                //else
                //{
                //    Vector3f.Cross(ref v3, ref v4, out temp2);
                //    Vector3f.Dot(ref temp2, ref direction, out dot);
                //    if (dot > 0)
                //    {
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3f direction, out float t, out Vector3f normal, out Vector3f position)
        {
            // Local surface cast is very similar to regular MPR.  However, instead of starting at an interior point and targeting the origin,
            // the ray starts at the origin (a point known to be in both shape and shapeB), and just goes towards the direction until the surface
            // is found.  The portal (v1, v2, v3) at termination defines the surface normal, and the distance from the origin to the portal along the direction is used as the 't' result.
            ...
            while (true)
            {
                ...
                if (supportDot - dot < surfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    Vector3f.Multiply(ref direction, t, out position);

                    Toolbox.GetBarycentricCoordinates(ref position, ref v1, ref v2, ref v3, out v1Weight, out v2Weight, out v3Weight);
                    Vector3f.Multiply(ref v1A, v1Weight, out position);
                    Vector3f temp;
                    Vector3f.Multiply(ref v2A, v2Weight, out temp);
                    Vector3f.Add(ref temp, ref position, out position);
                    Vector3f.Multiply(ref v3A, v3Weight, out temp);
                    Vector3f.Add(ref temp, ref position, out position);
                    ////DEBUG STUFF:
        ...
        static bool VerifySimplex(ref Vector3f v0, ref Vector3f v1, ref Vector3f v2, ref Vector3f v3, ref Vector3f direction)
        {


            //v1, v0, v3
            Vector3f cross = Vector3f.Cross(v0 - v1, v3 - v1);
            float planeProduct1 = Vector3f.Dot(cross, direction);
            //v3, v0, v2
            cross = Vector3f.Cross(v0 - v3, v2 - v3);
            float planeProduct2 = Vector3f.Dot(cross, direction);
            //v2, v0, v1
            cross = Vector3f.Cross(v0 - v2, v1 - v2);
            float planeProduct3 = Vector3f.Dot(cross, direction);
        ...
        public static bool GetContact(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB, ref Vector3f penetrationAxis, out ContactData contact)
        {
            RigidTransform localTransformB;
            MinkowskiToolbox.GetLocalTransform(ref transformA, ref transformB, out localTransformB);
            if (MPRToolbox.AreLocalShapesOverlapping(shapeA, shapeB, ref localTransformB))
            {
                ...
                Vector3f rayCastDirection;
                float lengthSquared = penetrationAxis.LengthSquared;
                if (lengthSquared > MathHelper.Epsilon)
                {
                    Vector3f.Divide(ref penetrationAxis, (float)Math.Sqrt(lengthSquared), out rayCastDirection);// (Vector3f.Normalize(localDirection) + Vector3f.Normalize(collidableB.worldTransform.Position - collidableA.worldTransform.Position)) / 2;
                ...
                Vector3f normalCandidate;
                float depthCandidate;
                lengthSquared = localTransformB.Position.LengthSquared;
                if (lengthSquared > MathHelper.Epsilon)
                {
                    Vector3f.Divide(ref localTransformB.Position, (float)Math.Sqrt(lengthSquared), out rayCastDirection);
                ...
                Matrix3f.FromQuaternion(ref transformA.Orientation, out orientation);
                Vector3f.Transform(ref contact.Normal, ref orientation, out contact.Normal);
                //Vector3f.Negate(ref contact.Normal, out contact.Normal);
                Vector3f.Transform(ref contact.Position, ref orientation, out contact.Position);
                Vector3f.Add(ref contact.Position, ref transformA.Position, out contact.Position);
                return true;
            }
        ...
        public static void RefinePenetration(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, float initialDepth, ref Vector3f initialNormal, out float penetrationDepth, out Vector3f refinedNormal, out Vector3f position)
        {
            //The local casting can optionally continue.  Eventually, it will converge to the local minimum.
        ...
        public static bool Sweep(ConvexShape shapeA, ConvexShape shapeB, ref Vector3f sweepA, ref Vector3f sweepB, ref RigidTransform transformA, ref RigidTransform transformB, out RayHit hit)
        {
            ...
            Vector3f.Subtract(ref sweepA, ref sweepB, out velocityWorld);
            ...
            Vector3f.Transform(ref velocityWorld, ref conjugateOrientationA, out localDirection);
            ...
            if (rayLengthSquared > MathHelper.Epsilon * .01f)
            {
                Vector3f.Dot(ref localTransformB.Position, ref localDirection, out sweepLength);
            ...
            Vector3f sweep;
            Vector3f.Multiply(ref localDirection, sweepLength, out sweep);
            ...
            if (negativeLength)
            {
                ...
                hit.T = 0;
                Vector3f.Normalize(ref localDirection, out hit.Normal);
                Vector3f.Transform(ref hit.Normal, ref transformA.Orientation, out hit.Normal);
                //hit.Location = hit.T * localDirection;
                Vector3f.Transform(ref hit.Location, ref transformA.Orientation, out hit.Location);
                Vector3f.Add(ref hit.Location, ref transformA.Position, out hit.Location);
            ...
            if (LocalSweepCast(shapeA, shapeB, sweepLength, rayLengthSquared, ref localDirection, ref sweep, ref localTransformB, out hit))
            {
                ...
                Vector3f minkowskiRayHit = -hit.T * localDirection;
                ...
                Vector3f temp;
                Vector3f.Multiply(ref sweepA, hit.T, out temp);
                Vector3f.Add(ref temp, ref hit.Location, out hit.Location);
                return true;
            }
            return false;
        ...
        private static bool LocalSweepCast(ConvexShape shape, ConvexShape shapeB, float sweepLength, float rayLengthSquared, ref Vector3f localDirection, ref Vector3f sweep, ref RigidTransform localTransformB, out RayHit hit)
        {
            ...
            //{
            //    hit.T = 0;
            //    Vector3f.Normalize(ref n, out hit.Normal);
            //    Vector3f.Transform(ref hit.Normal, ref transform.Orientation, out hit.Normal);
            //    //hit.Location = hit.T * localDirection;
            //    Vector3f.Transform(ref hit.Location, ref transform.Orientation, out hit.Location);
            //    Vector3f.Add(ref hit.Location, ref transform.Position, out hit.Location);
            //    hit.Location += sweepA * hit.T;
            //    return true;
            //}
            ...
            //Find another extreme point in a direction perpendicular to the previous.
            Vector3f v2, v2A;
            Vector3f.Cross(ref localDirection, ref v1, out n);
            hit.Location = new Vector3f();
            if (n.LengthSquared < MathHelper.Epsilon * .01f)
            {
                //v1 and v0 could be parallel.
                //This isn't a bad thing- it means the direction is exactly aligned with the extreme point offset.
                //In other words, if the raycast is followed out to the surface, it will arrive at the extreme point!
                ...
                if (rayLengthSquared > MathHelper.Epsilon * .01f)
                    Vector3f.Divide(ref localDirection, (float)Math.Sqrt(rayLengthSquared), out hit.Normal);
                else
                    hit.Normal = new Vector3f();

                float rate;
                Vector3f.Dot(ref  hit.Normal, ref localDirection, out rate);
                float distance;
                Vector3f.Dot(ref  hit.Normal, ref v1, out distance);
                if (rate > 0)
            ...
            //relative to the direction vector.
            float dot;
            Vector3f.Dot(ref n, ref localDirection, out dot);
            if (dot > 0)
            {
                //It's not properly calibrated.  Flip the winding (and the previously calculated normal).
                Vector3f.Negate(ref n, out n);
            ...
            while (true)
            {
                ...
                // If the direction is outside the plane defined by v1,v0,v3, then the portal is invalid.
                Vector3f.Cross(ref v1, ref v3, out temp1);
                Vector3f.Dot(ref temp1, ref localDirection, out dot);
                if (dot < 0)
                {
                    //Replace the point that was on the inside of the plane (v2) with the new extreme point.
                    v2 = v3;
            ...
            while (true)
            {
                ...
                if (supportDot - dot < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    float lengthSquared = n.LengthSquared;
                    if (lengthSquared > MathHelper.Epsilon * .00001f)
                    {
                        Vector3f.Divide(ref n, (float)Math.Sqrt(lengthSquared), out hit.Normal);

                        //The plane is very close to the surface, and the ray is known to pass through it.
                        //dot is the rate.
                        Vector3f.Dot(ref  hit.Normal, ref localDirection, out dot);
                        //supportDot is the distance to the plane.
                        Vector3f.Dot(ref  hit.Normal, ref v1, out supportDot);
                    ...
                    //Vector3f mdHitLocation = t * localDirection;
                    //float v1Weight, v2Weight, v3Weight;
                ...
                //This v4 x direction is just a minor reordering of a scalar triple product: (v1 x v4) * direction.
                //It eliminates the need for extra cross products for the inner if.
                Vector3f.Cross(ref v4, ref localDirection, out temp1);
                Vector3f.Dot(ref v1, ref temp1, out dot);
                if (dot >= 0)
                {
                    Vector3f.Dot(ref v2, ref temp1, out dot);
        ...
        internal static void GetLocalPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3f minkowskiPosition, out Vector3f position)
        {
            //Compute the ray.  This points from a point known to be inside the minkowski sum to the test minkowski position;
            //The centers of the shapes are used to create the interior point.
            Vector3f rayDirection;
            Vector3f.Add(ref minkowskiPosition, ref localTransformB.Position, out rayDirection);

            //It's possible that the point is extremely close to the A-B center.  In this case, early out.
            if (rayDirection.LengthSquared < MathHelper.Epsilon)
            {
            ...
            Vector3f v0;
            Vector3f.Negate(ref localTransformB.Position, out v0); //Since we're in A's local space, A-B is just -B.
            ...
            if (n.LengthSquared < MathHelper.Epsilon)
            {
                ...
                float dot = Vector3f.Dot(v1 - minkowskiPosition, rayDirection);
                //Vector3f.Dot(ref v1, ref rayDirection, out dot);
                ...
                //Origin is inside.
                //Compute barycentric coordinates along simplex (segment).
                float dotv0 = Vector3f.Dot(v0 - minkowskiPosition, rayDirection);
                //Dot > 0, so dotv0 starts out negative.
                //Vector3f.Dot(ref v0, ref rayDirection, out dotv0);
                float barycentricCoordinate = -dotv0 / (dot - dotv0);
                //Vector3f.Subtract(ref v1A, ref v0A, out offset); //'v0a' is just the zero vector, so there's no need to calculate the offset.
                Vector3f.Multiply(ref v1A, barycentricCoordinate, out position);
                Vector3f offset;
                Vector3f.Subtract(ref v1B, ref localTransformB.Position, out offset);
                return;
            }
            MinkowskiToolbox.GetLocalMinkowskiExtremePoint(shapeA, shapeB, ref n, ref localTransformB, out v2A, out v2B, out v2);
            ...
            Vector3f v0v1, v0v2;
            //Set n for the first iteration.
            Vector3f.Subtract(ref v1, ref v0, out v0v1);
            Vector3f.Subtract(ref v2, ref v0, out v0v2);
            Vector3f.Cross(ref v0v1, ref v0v2, out n);

            Vector3f pointToV0;
            Vector3f.Subtract(ref v0, ref minkowskiPosition, out pointToV0);
            Vector3f v3A, v3B, v3;
            int count = 0;
            while (true)
            {
                ...
                Vector3f v0v3;
                Vector3f.Subtract(ref v1, ref v0, out v0v1);
                Vector3f.Subtract(ref v3, ref v0, out v0v3);
                Vector3f.Cross(ref v0v1, ref v0v3, out temp);
                float dot;
                Vector3f.Dot(ref temp, ref pointToV0, out dot);
                if (dot < 0)
                {
                    //Replace the point that was on the inside of the plane (v2) with the new extreme point.
                    v2 = v3;
                    ...
                    v2B = v3B;
                    // Calculate the normal of the plane that will be used to find a new extreme point.
                    Vector3f.Cross(ref v0v1, ref v0v3, out n);
                    continue;
                }

                // If the origin is outside the plane defined by v3,v0,v2, then the portal is invalid.
                Vector3f.Subtract(ref v2, ref v0, out v0v2);
                Vector3f.Cross(ref v0v3, ref v0v2, out temp);
                Vector3f.Dot(ref temp, ref pointToV0, out dot);
                if (dot < 0)
                {
                    //Replace the point that was on the inside of the plane (v1) with the new extreme point.
                    ...
                    v1B = v3B;
                    // Calculate the normal of the plane that will be used to find a new extreme point.
                    Vector3f.Cross(ref v0v2, ref v0v3, out n);
                    continue;
                }
                break;
            }
            ...
            while (true)
            {
                ...
                //Compute the outward facing normal.
                Vector3f v2v3, v2v1;
                Vector3f.Subtract(ref v3, ref v2, out v2v3);
                Vector3f.Subtract(ref v1, ref v2, out v2v1);
                Vector3f.Cross(ref v2v3, ref v2v1, out n);
                float dot;
                Vector3f pointToV1;
                Vector3f.Subtract(ref v1, ref minkowskiPosition, out pointToV1);
                Vector3f.Dot(ref pointToV1, ref n, out dot);
                ...
                //    Vector3f.Cross(ref rayDirection, ref temp2, out cross);
                //    float v0ov2v3volume;
                //    Vector3f.Dot(ref cross, ref temp3, out v0ov2v3volume);

                //    Vector3f.Cross(ref temp1, ref rayDirection, out cross);
                //    float v0v1ov3volume;
                //    Vector3f.Dot(ref cross, ref temp3, out v0v1ov3volume);
                ...
                float dot2;
                Vector3f pointToV4;
                Vector3f.Subtract(ref v4, ref minkowskiPosition, out pointToV4);
                Vector3f.Dot(ref pointToV4, ref n, out dot2);
                ...
                if (dot2 - dot < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    Vector3f.Multiply(ref v1A, weight1, out position);
                    Vector3f.Multiply(ref v2A, weight2, out v2A);
                    Vector3f.Multiply(ref v3A, weight3, out v3A);
                    Vector3f.Add(ref v2A, ref position, out position);
                    Vector3f.Add(ref v3A, ref position, out position);
                ...
                Vector3f.Cross(ref pointToV4, ref pointToV0, out temp);
                Vector3f.Dot(ref pointToV1, ref temp, out dot);
                if (dot >= 0) //Dot v0 x v4 against v1, and dot it against 
                {
                    Vector3f pointToV2;
                    Vector3f.Subtract(ref v2, ref minkowskiPosition, out pointToV2);
                    Vector3f.Dot(ref pointToV2, ref temp, out dot);
                    if (dot >= 0)
                    {
                        v1 = v4; // Inside v1 & inside v2 ==> eliminate v1
                        v1A = v4A;
                ...
                {
                    Vector3f pointToV3;
                    Vector3f.Subtract(ref v3, ref minkowskiPosition, out pointToV3);
                    Vector3f.Dot(ref pointToV3, ref temp, out dot);
                    if (dot >= 0)
                    {
                        v2 = v4; // Outside v1 & inside v3 ==> eliminate v2
                        v2A = v4A;
        ...
        public static bool AreSweptShapesIntersecting(ConvexShape shapeA, ConvexShape shapeB, ref Vector3f sweep, ref RigidTransform localTransformB, out Vector3f position)
        {
            //It's possible that the two objects' centers are overlapping, or very very close to it.  In this case, 
            //they are obviously colliding and we can immediately exit.
            if (localTransformB.Position.LengthSquared < MathHelper.Epsilon)
            {
                position = new Vector3f();
                return true;
            }
            ...
            //This implementation is similar to that of the original XenoCollide.
            //'n' will be the direction used to find supports throughout the algorithm.
            Vector3f n = localTransformB.Position;
            Vector3f v1;
            Vector3f v1A; //extreme point contributions from each shape.  Used later to compute contact position; could be used to cache simplex too.
            ...
            if (n.LengthSquared < MathHelper.Epsilon)
            {
                ...
                //If the origin is within this extreme point, then there is an intersection.
                float dot;
                Vector3f.Dot(ref v1, ref localTransformB.Position, out dot);
                if (dot < 0)
                {
                    //Origin is outside.
                    position = new Vector3f();
                ...
                float dotv0;
                //Dot > 0, so dotv0 starts out negative.
                Vector3f.Dot(ref v0, ref localTransformB.Position, out dotv0);
                float barycentricCoordinate = -dotv0 / (dot - dotv0);
                //Vector3f.Subtract(ref v1A, ref v0A, out offset); //'v0a' is just the zero vector, so there's no need to calculate the offset.
                Vector3f.Multiply(ref v1A, barycentricCoordinate, out position);
                return true;
            ...
            while (true)
            {
                ...
                if (dot >= 0)
                {
                    ...
                    Vector3f.Cross(ref localTransformB.Position, ref temp2, out cross);
                    float v0ov2v3volume;
                    Vector3f.Dot(ref cross, ref temp3, out v0ov2v3volume);

                    Vector3f.Cross(ref temp1, ref localTransformB.Position, out cross);
                    float v0v1ov3volume;
                    Vector3f.Dot(ref cross, ref temp3, out v0v1ov3volume);
        ...
        static void GetSweptExtremePoint(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3f sweep, ref Vector3f extremePointDirection, out Vector3f extremePointA, out Vector3f extremePoint)
        {
            Vector3f b;
            ...
            float dot;
            Vector3f.Dot(ref extremePointDirection, ref sweep, out dot);
            if (dot > 0)
            {
                Vector3f.Add(ref extremePoint, ref sweep, out extremePoint);
            }
        }
        ...
        public static bool RayCast(Ray ray, float maximumLength, ConvexShape shape, ref RigidTransform transform, out RayHit hit)
        {
            ...
            Vector3f.Negate(ref localRay.Direction, out localRay.Direction);
            ...
            if (Toolbox.RayCastSphere(ref ray, ref transform.Position, shape.MaximumRadius, maximumLength, out sphereHit))
            {
                ...
                Vector3f offset;
                Vector3f.Multiply(ref localRay.Direction, -sphereHit.T, out offset);
                Vector3f.Add(ref localRay.Position, ref offset, out localRay.Position);
            }
            else
            {
            ...
            if (rayLengthSquared > MathHelper.Epsilon * .01f)
            {
                Vector3f.Dot(ref localRay.Position, ref localRay.Direction, out sweepLength);
            ...
            Vector3f sweep;
            Vector3f.Multiply(ref localRay.Direction, sweepLength, out sweep);
            //Check to see if the origin is contained within the swept shape.
            ...
            if (negativeLength)
            {
                ...
                hit.T = 0;
                Vector3f.Normalize(ref ray.Direction, out hit.Normal);
        ...
        public static bool SweptShapeContainsPoint(ConvexShape shape, ref Vector3f sweep, ref Vector3f localPoint)
        {
            //It's possible that the two objects' centers are overlapping, or very very close to it.  In this case, 
            //they are obviously colliding and we can immediately exit.
            if (localPoint.LengthSquared < MathHelper.Epsilon)
            {
                return true;
            }

            Vector3f v0;
            Vector3f.Negate(ref localPoint, out v0); //Since we're in A's local space, A-B is just -B.
            ...
            if (n.LengthSquared < MathHelper.Epsilon)
            {
                ...
                //If the origin is within this extreme point, then there is an intersection.
                float dot;
                Vector3f.Dot(ref v1, ref localPoint, out dot);
                if (dot < 0)
                {
                    //Origin is outside.
                    return false;
        ...
        private static bool LocalSweepCast(ConvexShape shape, float sweepLength, float rayLengthSquared, ref Vector3f localDirection, ref Vector3f sweep, ref Vector3f rayOrigin, out RayHit hit)
        {
            //By now, the ray is known to be within the swept shape and facing the right direction for a normal raycast.
        ...
        static void GetSweptExtremePoint(ConvexShape shape, ref Vector3f point, ref Vector3f sweep, ref Vector3f extremePointDirection, out Vector3f extremePoint)
        {
            shape.GetExtremePoint(extremePointDirection, ref MathHelper.RigidIdentity, out extremePoint);
            Vector3f.Subtract(ref extremePoint, ref point, out extremePoint);
            float dot;
            Vector3f.Dot(ref extremePointDirection, ref sweep, out dot);
            if (dot > 0)
            {
