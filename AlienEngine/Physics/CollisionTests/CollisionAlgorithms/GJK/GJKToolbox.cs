...
namespace AlienEngine.Core.Physics.CollisionTests.CollisionAlgorithms.GJK
{
    ...
    public static class GJKToolbox
    {
        ...
        public static bool AreShapesIntersecting(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB)
        {
            ...
            Vector3f separatingAxis = Toolbox.ZeroVector;
        ...
        public static bool AreShapesIntersecting(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB,
                                                 ref Vector3f localSeparatingAxis)
        {
            ...
            while (count++ < MaximumGJKIterations)
            {
                if (simplex.GetPointClosestToOrigin(out closestPoint) || //Also reduces the simplex.
                    closestPoint.LengthSquared <= simplex.GetErrorTolerance() * MathHelper.BigEpsilon)
                {
                ...
                Vector3f direction;
                Vector3f.Negate(ref closestPoint, out direction);
                ...
                float dot;
                Vector3f.Dot(ref extremePoint, ref closestPoint, out dot); //extreme point dotted against the direction pointing backwards towards the CSO. 
                if (dot > 0)
                {
        ...
        public static bool GetClosestPoints(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB,
                                            out Vector3f closestPointA, out Vector3f closestPointB)
        {
        ...
        ///<returns>Whether or not the objects were intersecting.  If they are intersecting, then the closest points cannot be identified.</returns>
        public static bool GetClosestPoints(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB,
                                            ref CachedSimplex cachedSimplex, out Vector3f closestPointA, out Vector3f closestPointB)
        {
            RigidTransform localtransformB;
            MinkowskiToolbox.GetLocalTransform(ref transformA, ref transformB, out localtransformB);
        ...
        private static bool GetClosestPoints(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB,
                                             ref CachedSimplex cachedSimplex, out Vector3f localClosestPointA, out Vector3f localClosestPointB)
        {
            ...
            while (true)
            {
                if (simplex.GetPointClosestToOrigin(out closestPoint) || //Also reduces the simplex and computes barycentric coordinates if necessary. 
                    closestPoint.LengthSquared <= MathHelper.Epsilon * simplex.errorTolerance)
                {
                    //Intersecting.
            ...
            //if (Vector3f.Distance(localClosestPointA - localClosestPointB, closestPoint) > .00001f)
                                   ...
                                   out RayHit hit)
        {
            ...
            Vector3f.Subtract(ref ray.Position, ref shapeTransform.Position, out ray.Position);
            Quaternion conjugate;
            Quaternion.Conjugate(ref shapeTransform.Orientation, out conjugate);
            Vector3f.Transform(ref ray.Position, ref conjugate, out ray.Position);
            Vector3f.Transform(ref ray.Direction, ref conjugate, out ray.Direction);

            Vector3f extremePointToRayOrigin, extremePoint;
            hit.T = 0;
            hit.Location = ray.Position;
            hit.Normal = Toolbox.ZeroVector;
            Vector3f closestOffset = hit.Location;
            ...
            while (closestOffset.LengthSquared >= MathHelper.Epsilon * simplex.GetErrorTolerance(ref ray.Position))
            {
                if (++count > MaximumGJKIterations)
                {
                ...
                Vector3f.Subtract(ref hit.Location, ref extremePoint, out extremePointToRayOrigin);
                Vector3f.Dot(ref closestOffset, ref extremePointToRayOrigin, out vw);
                ...
                if (vw > 0)
                {
                    ...
                    Vector3f.Dot(ref closestOffset, ref ray.Direction, out closestPointDotDirection);
            ...
            Vector3f.Transform(ref hit.Normal, ref shapeTransform.Orientation, out hit.Normal);
            Vector3f.Transform(ref hit.Location, ref shapeTransform.Orientation, out hit.Location);
            Vector3f.Add(ref hit.Location, ref shapeTransform.Position, out hit.Location);
        ...
        public static bool ConvexCast(ConvexShape sweptShape, ConvexShape target, ref Vector3f sweep, ref RigidTransform startingSweptTransform, ref RigidTransform targetTransform,
                                  out RayHit hit)
        {
        ...
        public static bool ConvexCast(ConvexShape shapeA, ConvexShape shapeB, ref Vector3f sweepA, ref Vector3f sweepB, ref RigidTransform transformA, ref RigidTransform transformB,
                                  out RayHit hit)
        {
            //Put the velocity into shapeA's local space.
            Vector3f velocityWorld;
            Vector3f.Subtract(ref sweepB, ref sweepA, out velocityWorld);
            Quaternion conjugateOrientationA;
            Quaternion.Conjugate(ref transformA.Orientation, out conjugateOrientationA);
            Vector3f rayDirection;
            Vector3f.Transform(ref velocityWorld, ref conjugateOrientationA, out rayDirection);
            ...
            Vector3f w, p;
            hit.T = 0;
            hit.Location = Vector3f.Zero; //The ray starts at the origin.
            hit.Normal = Toolbox.ZeroVector;
            Vector3f v = hit.Location;
            ...
            do
            {
                ...
                Vector3f.Subtract(ref hit.Location, ref p, out w);
                Vector3f.Dot(ref v, ref w, out vw);
                if (vw > 0)
                {
                    Vector3f.Dot(ref v, ref rayDirection, out vdir);
                    if (vdir >= 0)
                    {
                        hit = new RayHit();
                        return false;
                    ...
                    //Shift the ray up.
                    Vector3f.Multiply(ref rayDirection, hit.T, out hit.Location);
            ...
            } while (v.LengthSquared >= MathHelper.Epsilon * simplex.GetErrorTolerance(ref Toolbox.ZeroVector));
            //This epsilon has a significant impact on performance and accuracy.  Changing it to use BigEpsilon instead increases speed by around 30-40% usually, but jigging is more evident.
            //Transform the hit data into world space.
            Vector3f.Transform(ref hit.Normal, ref transformA.Orientation, out hit.Normal);
            Vector3f.Multiply(ref velocityWorld, hit.T, out hit.Location);
                                   ...
                                   out RayHit hit)
        {
            ...
            //This epsilon has a significant impact on performance and accuracy.  Changing it to use BigEpsilon instead increases speed by around 30-40% usually, but jigging is more evident.
            while (v.LengthSquared >= MathHelper.Epsilon * simplex.GetErrorTolerance(ref ray.Position))
            {
                if (++count > MaximumGJKIterations)
                {
                    //It's taken too long to find a hit.  Numerical problems are probable; quit.
                ...
                Vector3f.Add(ref p, ref contribution, out p);

                Vector3f.Subtract(ref hit.Location, ref p, out w);
                Vector3f.Dot(ref v, ref w, out vw);
                if (vw > 0)
                {
                    Vector3f.Dot(ref v, ref ray.Direction, out vdir);
                    hit.T = hit.T - vw / vdir;
                    if (vdir >= 0)
                    {
