...
namespace AlienEngine.Core.Physics.CollisionTests.CollisionAlgorithms.GJK
{
    ...
    public struct RaySimplex
    {
        ...
        public bool GetPointClosestToOrigin(ref RaySimplex simplex, out Vector3f point)
        {
        ...
        public void GetPointOnSegmentClosestToOrigin(ref RaySimplex simplex, out Vector3f point)
        {
        ...
        public void GetPointOnTriangleClosestToOrigin(ref RaySimplex simplex, out Vector3f point)
        {
            Vector3f ab, ac;
        ...
        public bool GetPointOnTetrahedronClosestToOrigin(ref RaySimplex simplex, out Vector3f point)
        {
        ...
        private static bool TryTetrahedronTriangle(ref Vector3f A, ref Vector3f B, ref Vector3f C,
                                                   ref Vector3f simplexA, ref Vector3f simplexB, ref Vector3f simplexC,
                                                   ref Vector3f otherPoint, out RaySimplex simplex, out Vector3f point)
        {
            ...
            if (AdotN * ADdotN >= 0)
            {
                ...
                if (va <= 0f && (d3d4 = BdotAC - BdotAB) > 0f && (d6d5 = CdotAB - CdotAC) > 0f)//Note > instead of >= and < instead of <=; prevents bad denominator
                {
                    ...
                    Vector3f bc;
                    Vector3f.Subtract(ref C, ref B, out bc);
                    Vector3f.Multiply(ref bc, V, out point);
                    Vector3f.Add(ref point, ref B, out point);
                    return true;
                }
        ...
        public void AddNewSimplexPoint(ref Vector3f point, ref Vector3f hitLocation, out RaySimplex shiftedSimplex)
        {
            ...
            switch (State)
            {
                case SimplexState.Empty:
                    ...
                    Vector3f.Subtract(ref hitLocation, ref A, out shiftedSimplex.A);
                    break;
                case SimplexState.Point:
                    ...
                    Vector3f.Subtract(ref hitLocation, ref A, out shiftedSimplex.A);
                    Vector3f.Subtract(ref hitLocation, ref B, out shiftedSimplex.B);
                    break;
                case SimplexState.Segment:
                    ...
                    Vector3f.Subtract(ref hitLocation, ref A, out shiftedSimplex.A);
                    Vector3f.Subtract(ref hitLocation, ref B, out shiftedSimplex.B);
                    Vector3f.Subtract(ref hitLocation, ref C, out shiftedSimplex.C);
                    break;
                case SimplexState.Triangle:
                    ...
                    Vector3f.Subtract(ref hitLocation, ref B, out shiftedSimplex.B);
                    Vector3f.Subtract(ref hitLocation, ref C, out shiftedSimplex.C);
                    Vector3f.Subtract(ref hitLocation, ref D, out shiftedSimplex.D);
                    break;
            }
        ...
        public float GetErrorTolerance(ref Vector3f rayOrigin)
        {
            switch (State)
            {
                case SimplexState.Point:
                    float distanceA;
                    Vector3f.DistanceSquared(ref A, ref rayOrigin, out distanceA);
                    return distanceA;
                case SimplexState.Segment:
                    float distanceB;
                    Vector3f.DistanceSquared(ref A, ref rayOrigin, out distanceA);
                    Vector3f.DistanceSquared(ref B, ref rayOrigin, out distanceB);
                    return MathHelper.Max(distanceA, distanceB);
                case SimplexState.Triangle:
                    float distanceC;
                    Vector3f.DistanceSquared(ref A, ref rayOrigin, out distanceA);
                    Vector3f.DistanceSquared(ref B, ref rayOrigin, out distanceB);
                    Vector3f.DistanceSquared(ref C, ref rayOrigin, out distanceC);
                ...
                case SimplexState.Tetrahedron:
                    ...
                    Vector3f.DistanceSquared(ref B, ref rayOrigin, out distanceB);
                    Vector3f.DistanceSquared(ref C, ref rayOrigin, out distanceC);
                    Vector3f.DistanceSquared(ref D, ref rayOrigin, out distanceD);
