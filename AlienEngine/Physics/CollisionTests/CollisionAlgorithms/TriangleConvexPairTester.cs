...
using AlienEngine.Core.Physics.CollisionTests.CollisionAlgorithms.GJK;
...
namespace AlienEngine.Core.Physics.CollisionTests.CollisionAlgorithms
{
    ...
    public class TriangleConvexPairTester : TrianglePairTester
    {
        ...
        private bool DoPlaneTest(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            Vector3f.Cross(ref ac, ref ab, out reverseNormal);
            ...
            float dotA;
            Vector3f.Dot(ref triangle.vA, ref reverseNormal, out dotA);
            ...
            switch (triangle.sidedness)
            {
                case TriangleSidedness.DoubleSided:
                    if (dotA < 0)
                    {
                        ...
                        Vector3f.Negate(ref reverseNormal, out reverseNormal);
                        dotA = -dotA;
                    }
                    break;
            ...
            float dotE;
            Vector3f.Dot(ref extremePoint, ref reverseNormal, out dotE);
            float t = (dotA - dotE) / reverseNormal.LengthSquared;



            Vector3f offset;
            Vector3f.Multiply(ref reverseNormal, t, out offset);
            ...
            if (t <= 0 || distanceSquared < marginSum * marginSum)
            {
                ...
                if (marginSum > MathHelper.Epsilon) //This can be zero! It would cause a NaN is unprotected.
                    Vector3f.Multiply(ref offset, convex.collisionMargin / marginSum, out contact.Position); //t * AB
                else contact.Position = new Vector3f();
                Vector3f.Add(ref extremePoint, ref contact.Position, out contact.Position); //A + t * AB.

                float normalLength = reverseNormal.Length;
                Vector3f.Divide(ref reverseNormal, normalLength, out contact.Normal);
        ...
        private bool DoExternalSeparated(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            if (GJKToolbox.AreShapesIntersecting(convex, triangle, ref MathHelper.RigidIdentity, ref MathHelper.RigidIdentity, ref localSeparatingAxis))
            {
                state = CollisionState.ExternalNear;
                return DoExternalNear(triangle, out contactList);
            }
        ...
        private bool DoExternalNear(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            Vector3f.Add(ref triangle.vA, ref triangle.vB, out triangleCentroid);
            Vector3f.Add(ref triangleCentroid, ref triangle.vC, out triangleCentroid);
            Vector3f.Multiply(ref triangleCentroid, .33333333f, out triangleCentroid);

            var initialSimplex = new CachedSimplex { State = SimplexState.Point, LocalSimplexB = { A = triangleCentroid } };
            if (GJKToolbox.GetClosestPoints(convex, triangle, ref MathHelper.RigidIdentity, ref MathHelper.RigidIdentity, ref initialSimplex, out closestA, out closestB))
            {
                state = CollisionState.Deep;
                return DoDeepContact(triangle, out contactList);
            }
            Vector3f displacement;
            Vector3f.Subtract(ref closestB, ref closestA, out displacement);
            ...
            if (distanceSquared < margin * margin)
            {
                ...
                {
                    Vector3f triangleNormal, ab, ac;
                    Vector3f.Subtract(ref triangle.vB, ref triangle.vA, out ab);
                    Vector3f.Subtract(ref triangle.vC, ref triangle.vA, out ac);
                    Vector3f.Cross(ref ab, ref ac, out triangleNormal);
                    float dot;
                    Vector3f.Dot(ref triangleNormal, ref displacement, out dot);
                ...
                //Displacement is from A to B.  point = A + t * AB, where t = marginA / margin.
                if (margin > MathHelper.Epsilon) //This can be zero! It would cause a NaN if unprotected.
                    Vector3f.Multiply(ref displacement, convex.collisionMargin / margin, out contact.Position); //t * AB
                else contact.Position = new Vector3f();
                Vector3f.Add(ref closestA, ref contact.Position, out contact.Position); //A + t * AB.



                contact.Normal = displacement;
                float distance = (float)Math.Sqrt(distanceSquared);
                Vector3f.Divide(ref contact.Normal, distance, out contact.Normal);
        ...
        private bool DoDeepContact(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            Vector3f center;
            Vector3f.Add(ref triangle.vA, ref triangle.vB, out center);
            Vector3f.Add(ref center, ref triangle.vC, out center);
            ...
            if (MPRToolbox.AreLocalShapesOverlapping(convex, triangle, ref center, ref MathHelper.RigidIdentity))
            {
                ...
                if (lengthSquared < MathHelper.Epsilon * .01f)
                {
                    ...
                    MPRToolbox.LocalSurfaceCast(convex, triangle, ref MathHelper.RigidIdentity, ref center, out contact.PenetrationDepth, out contact.Normal, out contact.Position);
                }
                else
                {
                    //Normalize the normal.
                    Vector3f.Divide(ref triangleNormal, (float)Math.Sqrt(lengthSquared), out triangleNormal);
                    ...
                    Vector3f AO, BO, CO;
                    Vector3f AB, BC, CA;
                    Vector3f.Subtract(ref center, ref triangle.vA, out AO);
                    Vector3f.Subtract(ref center, ref triangle.vB, out BO);
                    Vector3f.Subtract(ref center, ref triangle.vC, out CO);
                    Vector3f.Subtract(ref triangle.vB, ref triangle.vA, out AB);
                    Vector3f.Subtract(ref triangle.vC, ref triangle.vB, out BC);
                    Vector3f.Subtract(ref triangle.vA, ref triangle.vC, out CA);


                    //We don't have to worry about degenerate triangles here because we've already handled that possibility above.
                    Vector3f ABnormal, BCnormal, CAnormal;

                    //Project the center onto the edge to find the direction from the center to the edge AB.
                    Vector3f.Dot(ref AO, ref AB, out dot);
                    Vector3f.Multiply(ref AB, dot / AB.LengthSquared, out ABnormal);
                    Vector3f.Subtract(ref AO, ref ABnormal, out ABnormal);
                    ABnormal.Normalize();

                    //Project the center onto the edge to find the direction from the center to the edge BC.
                    Vector3f.Dot(ref BO, ref BC, out dot);
                    Vector3f.Multiply(ref BC, dot / BC.LengthSquared, out BCnormal);
                    Vector3f.Subtract(ref BO, ref BCnormal, out BCnormal);
                    BCnormal.Normalize();

                    //Project the center onto the edge to find the direction from the center to the edge BC.
                    Vector3f.Dot(ref CO, ref CA, out dot);
                    Vector3f.Multiply(ref CA, dot / CA.LengthSquared, out CAnormal);
                    Vector3f.Subtract(ref CO, ref CAnormal, out CAnormal);
                    CAnormal.Normalize();


                    MPRToolbox.LocalSurfaceCast(convex, triangle, ref MathHelper.RigidIdentity, ref ABnormal, out contact.PenetrationDepth, out contact.Normal);
                    //Check to see if the normal is facing in the proper direction, considering that this may not be a two-sided triangle.
                    Vector3f.Dot(ref triangleNormal, ref contact.Normal, out dot);
                    if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > 0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < 0))
                    {
                        ...
                        Vector3f previousNormal = contact.Normal;
                        Vector3f.Dot(ref contact.Normal, ref triangleNormal, out dot);

                        Vector3f p;
                        Vector3f.Multiply(ref contact.Normal, dot, out p);
                        Vector3f.Subtract(ref contact.Normal, ref p, out contact.Normal);
                        float length = contact.Normal.LengthSquared;
                        if (length > MathHelper.Epsilon)
                        {
                            //Renormalize the corrected normal.
                            Vector3f.Divide(ref contact.Normal, (float)Math.Sqrt(length), out contact.Normal);
                            Vector3f.Dot(ref contact.Normal, ref previousNormal, out dot);
                    ...
                    MPRToolbox.LocalSurfaceCast(convex, triangle, ref MathHelper.RigidIdentity, ref BCnormal, out candidateDepth, out candidateNormal);
                    //Check to see if the normal is facing in the proper direction, considering that this may not be a two-sided triangle.
                    Vector3f.Dot(ref triangleNormal, ref candidateNormal, out dot);
                    if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > 0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < 0))
                    {
                        //Normal was facing the wrong way.
                        //Instead of ignoring it entirely, correct the direction to as close as it can get by removing any component parallel to the triangle normal.
                        Vector3f previousNormal = candidateNormal;
                        Vector3f.Dot(ref candidateNormal, ref triangleNormal, out dot);

                        Vector3f p;
                        Vector3f.Multiply(ref candidateNormal, dot, out p);
                        Vector3f.Subtract(ref candidateNormal, ref p, out candidateNormal);
                        float length = candidateNormal.LengthSquared;
                        if (length > MathHelper.Epsilon)
                        {
                            //Renormalize the corrected normal.
                            Vector3f.Divide(ref candidateNormal, (float)Math.Sqrt(length), out candidateNormal);
                            Vector3f.Dot(ref candidateNormal, ref previousNormal, out dot);
                            candidateDepth *= dot;
                        }
                        else
                        {
                    ...
                    MPRToolbox.LocalSurfaceCast(convex, triangle, ref MathHelper.RigidIdentity, ref CAnormal, out candidateDepth, out candidateNormal);
                    //Check to see if the normal is facing in the proper direction, considering that this may not be a two-sided triangle.
                    Vector3f.Dot(ref triangleNormal, ref candidateNormal, out dot);
                    if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > 0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < 0))
                    {
                    ...
                    {
                        MPRToolbox.LocalSurfaceCast(convex, triangle, ref MathHelper.RigidIdentity, ref triangleNormal, out candidateDepth, out candidateNormal);
                        if (candidateDepth < contact.PenetrationDepth)
                        {
                            contact.Normal = candidateNormal;
                            contact.PenetrationDepth = candidateDepth;
                    ...
                    {
                        Vector3f.Negate(ref triangleNormal, out triangleNormal);
                        MPRToolbox.LocalSurfaceCast(convex, triangle, ref MathHelper.RigidIdentity, ref triangleNormal, out candidateDepth, out candidateNormal);
                        if (candidateDepth < contact.PenetrationDepth)
                        {
                            contact.Normal = candidateNormal;
                ...
                MPRToolbox.RefinePenetration(convex, triangle, ref MathHelper.RigidIdentity, contact.PenetrationDepth, ref contact.Normal, out contact.PenetrationDepth, out contact.Normal, out contact.Position);
        ...
        void TryToEscape(TriangleShape triangle, ref Vector3f position)
        {
        ...
        private bool TryInnerSphereContact(TriangleShape triangle, out ContactData contact)
        {
            ...
            if (length < minimumRadius * minimumRadius)
            {
                ...
                Vector3f.Cross(ref ab, ref ac, out triangleNormal);
                float dot;
                Vector3f.Dot(ref closestPoint, ref triangleNormal, out dot);
                if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > 0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < 0))
                {
                    //Normal was facing the wrong way.
                ...
                if (length > MathHelper.Epsilon) //Watch out for NaN's!
                {
                    Vector3f.Divide(ref closestPoint, length, out contact.Normal);
                }
                else
                {
                    ...
                    if (triangleNormal.LengthSquared > MathHelper.Epsilon)
                    {
                        Vector3f.Divide(ref triangleNormal, (float)Math.Sqrt(normalLength), out triangleNormal);
                        if (triangle.sidedness == TriangleSidedness.Clockwise)
        ...
        private VoronoiRegion GetVoronoiRegion(TriangleShape triangle, ref Vector3f p)
        {
            ...
            Vector3f.Subtract(ref triangle.vB, ref triangle.vA, out ab);
            Vector3f.Subtract(ref triangle.vC, ref triangle.vA, out ac);
            Vector3f.Subtract(ref p, ref triangle.vA, out ap);

            //Check to see if it's outside A.
            float APdotAB, APdotAC;
            Vector3f.Dot(ref ap, ref ab, out APdotAB);
            Vector3f.Dot(ref ap, ref ac, out APdotAC);
            ...
            Vector3f bp;
            Vector3f.Subtract(ref p, ref triangle.vB, out bp);
            Vector3f.Dot(ref ab, ref bp, out BPdotAB);
            Vector3f.Dot(ref ac, ref bp, out BPdotAC);
            ...
            Vector3f cp;
            Vector3f.Subtract(ref p, ref triangle.vC, out cp);
            Vector3f.Dot(ref ab, ref cp, out CPdotAB);
            Vector3f.Dot(ref ac, ref cp, out CPdotAC);
        ...
        public override void CleanUp()
        {
            ...
            state = CollisionState.Plane;
            escapeAttempts = 0;
            localSeparatingAxis = new Vector3f();
            Updated = false;
        }
        ...
        public override VoronoiRegion GetRegion(TriangleShape triangle, ref ContactData contact)
        {
            ...
            Vector3f.Dot(ref triangle.vA, ref contact.Normal, out dotA);
            Vector3f.Dot(ref triangle.vB, ref contact.Normal, out dotB);
            Vector3f.Dot(ref triangle.vC, ref contact.Normal, out dotC);
            ...
            if (dotA > dotB && dotA > dotC)
            {
                ...
                if (dotB > dotC)
                {
                    ...
                    {
                        Vector3f.Subtract(ref triangle.vB, ref triangle.vA, out edgeDirection);
                        Vector3f.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                ...
                else
                {
                    ...
                    {
                        Vector3f.Subtract(ref triangle.vC, ref triangle.vA, out edgeDirection);
                        Vector3f.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared * edgeEpsilon)
                            return VoronoiRegion.AC;
                        else
            ...
            else if (dotB > dotC)
            {
                ...
                if (dotC > dotA)
                {
                    ...
                    {
                        Vector3f.Subtract(ref triangle.vC, ref triangle.vB, out edgeDirection);
                        Vector3f.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared * edgeEpsilon)
                            return VoronoiRegion.BC;
                        else
                ...
                else
                {
                    ...
                    {
                        Vector3f.Subtract(ref triangle.vA, ref triangle.vB, out edgeDirection);
                        Vector3f.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared * edgeEpsilon)
                            return VoronoiRegion.AB;
                        else
            ...
            else
            {
                ...
                if (dotA > dotB)
                {
                    ...
                    {
                        Vector3f.Subtract(ref triangle.vA, ref triangle.vC, out edgeDirection);
                        Vector3f.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared * edgeEpsilon)
                            return VoronoiRegion.AC;
                        else
                ...
                else
                {
                    ...
                    {
                        Vector3f.Subtract(ref triangle.vB, ref triangle.vC, out edgeDirection);
                        Vector3f.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared * edgeEpsilon)
                            return VoronoiRegion.BC;
                        else
