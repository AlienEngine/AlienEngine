...
namespace AlienEngine.Core.Physics.CollisionTests.Manifolds
{
    ...
    public abstract class TriangleMeshConvexContactManifold : ContactManifold
    {
        ...
        public override void Update(float dt)
        {
            ...
            convex.Shape.GetBoundingBox(ref MathHelper.RigidIdentity, out convexLocalBoundingBox);
            ...
            if (UseImprovedBoundaryHandling)
            {
                ...
                if (boundarySets.VertexContacts.Count == 0 && guaranteedContacts == 0 && boundarySets.EdgeContacts.Count > 1)
                {
                    ...
                    float dot;
                    Vector3f.Dot(ref firstNormal, ref boundarySets.EdgeContacts.Elements[0].CorrectedNormal, out dot);
                    if (Math.Abs(dot) > .01f)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            Vector3f.Dot(ref boundarySets.EdgeContacts.Elements[i].ContactData.Normal, ref firstNormal, out dot);
                            if (dot < 0)
                            {
                                atLeastOneNormalAgainst = true;
                            }
                            //Check to see if the normal is outside the plane.
                            Vector3f.Dot(ref boundarySets.EdgeContacts.Elements[i].ContactData.Normal, ref boundarySets.EdgeContacts.Elements[0].CorrectedNormal, out dot);
                    ...
                    if (allNormalsInSamePlane && atLeastOneNormalAgainst)
                    {
                        ...
                        for (int i = 1; i < boundarySets.EdgeContacts.Count; i++)
                        {
                            ...
                            Vector3f.Dot(ref boundarySets.EdgeContacts.Elements[i].CorrectedNormal, ref boundarySets.EdgeContacts.Elements[i].ContactData.Normal, out dot);
                            if (dot < .01)
                            {
                ...
                for (int i = 0; i < boundarySets.VertexContacts.Count; i++)
                {
                    ...
                    else if (boundarySets.VertexContacts.Elements[i].ShouldCorrect || guaranteedContacts == 0)
                    {
                        ...
                        Vector3f.Dot(ref boundarySets.VertexContacts.Elements[i].CorrectedNormal, ref boundarySets.VertexContacts.Elements[i].ContactData.Normal, out dot);
        ...
        protected void GetNormal(ref Vector3f uncorrectedNormal, TriangleShape localTriangleShape, out Vector3f normal)
        {
            ...
            Vector3f AB, AC;
            Vector3f.Subtract(ref localTriangleShape.vB, ref localTriangleShape.vA, out AB);
            Vector3f.Subtract(ref localTriangleShape.vC, ref localTriangleShape.vA, out AC);
            ...
            switch (localTriangleShape.sidedness)
            {
                case TriangleSidedness.DoubleSided:
                    ...
                    Vector3f.Dot(ref normal, ref uncorrectedNormal, out dot);
                    if (dot < 0)
            ...
            if (normal.LengthSquared < MathHelper.Epsilon)
        ...
        private bool IsContactUnique(ref ContactData contactCandidate, ref QuickList<ContactData> candidatesToAdd)
        {
            ...
            for (int i = 0; i < contacts.Count; i++)
            {
                ...
                if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
                {
                    ...
                    Vector3f.Dot(ref contacts.Elements[i].Normal, ref contactCandidate.Normal, out distanceSquared);
            ...
            for (int i = 0; i < candidatesToAdd.Count; i++)
            {
                Vector3f.DistanceSquared(ref candidatesToAdd.Elements[i].Position, ref contactCandidate.Position, out distanceSquared);
                if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
                {
                    //This is a nonconvex manifold.  There will be times where a an object will be shoved into a corner such that
                    //a single position will have two reasonable normals.  If the normals aren't mostly aligned, they should NOT be considered equivalent.
                    Vector3f.Dot(ref candidatesToAdd.Elements[i].Normal, ref contactCandidate.Normal, out distanceSquared);
                    if (Math.Abs(distanceSquared) >= CollisionDetectionSettings.nonconvexNormalDotMinimum)
                        return false;
                }
            }
            ...
            //{
            //    Vector3f.DistanceSquared(ref edgeContacts.Elements[i].ContactData.Position, ref contactCandidate.Position, out distanceSquared);
            //    if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
            //    {
            //        return false;
            //    }
            ...
            //{
            //    Vector3f.DistanceSquared(ref vertexContacts.Elements[i].ContactData.Position, ref contactCandidate.Position, out distanceSquared);
            //    if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
            //    {
            //        return false;
            //    }
        ...
        struct EdgeContact
        {
            ...
            public Vector3f CorrectedNormal;
            public Edge Edge;
