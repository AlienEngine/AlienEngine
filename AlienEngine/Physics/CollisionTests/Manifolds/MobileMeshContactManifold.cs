    ...
    public abstract class MobileMeshContactManifold : TriangleMeshConvexContactManifold
    {
        ...
        protected internal override int FindOverlappingTriangles(float dt)
        {
            ...
            if (convex.entity != null)
                ...
                transformedVelocity = new Vector3f();
            if (mesh.entity != null)
                Vector3f.Subtract(ref transformedVelocity, ref mesh.entity.linearVelocity, out transformedVelocity);

            //The linear transform is known to be orientation only, so using the transpose is allowed.
            Vector3f.TransformTranspose(ref transformedVelocity, ref transform.LinearTransform, out transformedVelocity);
            Vector3f.Multiply(ref transformedVelocity, dt, out transformedVelocity);
        ...
        float previousDepth;
        Vector3f lastValidConvexPosition;
        protected override void ProcessCandidates(ref QuickList<ContactData> candidates)
        {
            if (candidates.Count == 0 && parentContactCount == 0 && Mesh.Shape.solidity == MobileMeshSolidity.Solid)
            {
                ...
                Matrix3f.FromQuaternion(ref mesh.worldTransform.Orientation, out orientation);

                Ray ray;
                Vector3f.Subtract(ref convex.worldTransform.Position, ref mesh.worldTransform.Position, out ray.Position);
                Vector3f.TransformTranspose(ref ray.Position, ref orientation, out ray.Position);

                //Cast from the current position back to the previous position.
                Vector3f.Subtract(ref lastValidConvexPosition, ref ray.Position, out ray.Direction);
                float rayDirectionLength = ray.Direction.LengthSquared;
                if (rayDirectionLength < MathHelper.Epsilon)
                {
                ...
                Vector3f.Divide(ref ray.Direction, (float)Math.Sqrt(rayDirectionLength), out ray.Direction);
                ...
                if (mesh.Shape.IsLocalRayOriginInMesh(ref ray, out hit))
                {
                    ...
                    Vector3f.Transform(ref ray.Position, ref orientation, out newContact.Position);
                    Vector3f.Add(ref newContact.Position, ref mesh.worldTransform.Position, out newContact.Position);

                    newContact.Normal = hit.Normal;
                    newContact.Normal.Normalize();

                    float factor;
                    Vector3f.Dot(ref ray.Direction, ref newContact.Normal, out factor);
                    newContact.PenetrationDepth = -factor * hit.T + convex.Shape.MinimumRadius;

                    Vector3f.Transform(ref newContact.Normal, ref orientation, out newContact.Normal);
                    ...
                    for (int i = 0; i < contacts.Count; i++)
                    {
                        if (contacts.Elements[i].Id == 2)
                        {
                            ...
                            supplementData.Elements[i].LocalOffsetA = new Vector3f();
