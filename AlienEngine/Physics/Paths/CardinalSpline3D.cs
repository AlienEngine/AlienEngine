...
namespace AlienEngine.Core.Physics.Paths
{
    ...
    public class CardinalSpline3D : HermiteCurve3D
    {
        ...
        protected override void ComputeTangents()
        {
            ...
            {
                Vector3f tangent;
                Vector3f previous = ControlPoints[i - 1].Value;
                Vector3f next = ControlPoints[i + 1].Value;
                Vector3f.Subtract(ref next, ref previous, out tangent);
                Vector3f.Multiply(ref tangent, (float)((1 - tension) / (ControlPoints[i + 1].Time - ControlPoints[i - 1].Time)), out tangent);
