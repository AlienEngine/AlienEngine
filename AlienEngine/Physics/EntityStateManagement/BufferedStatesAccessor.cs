...
namespace AlienEngine.Core.Physics.EntityStateManagement
{
    ///<summary>
    ...
    public class BufferedStatesAccessor
    {
        ...
        public Matrix3f OrientationMatrix
        {
            get
            {
                ...
                if (IsReadBufferAccessible())
                {
                    ...
                    Matrix3f.FromQuaternion(ref o, out toReturn);
                }
                else
                    Matrix3f.FromQuaternion(ref bufferedStates.Entity.orientation, out toReturn);
                return toReturn;
            }
            set
            {
                if (IsWriteBufferAccessible())
                {
                    Quaternion toSet = Quaternion.Normalize(Quaternion.FromRotationMatrix(value));
        ...
        public Matrix4f WorldTransform
        {
            ...
            set
            {
                if (IsWriteBufferAccessible())
                {
                    Vector3f translation = value.Translation;
