...
namespace AlienEngine.Core.Physics.Ik
{
    ...
    public class Bone
    {
        ...
        public Bone(Vector3f position, Quaternion orientation, float radius, float height, float mass)
        ...
        /// <param name="radius">Radius of the bone.</param>
        /// <param name="height">Height of the bone.</param>
        public Bone(Vector3f position, Quaternion orientation, float radius, float height)
        {
            Mass = 1;
            Position = position;
        ...
        {
            var localInertiaTensor = new Matrix3f();
            ...
            Matrix3f.Invert(ref localInertiaTensor, out localInertiaTensorInverse);
        }
        ...
        internal void UpdatePosition()
        {
            ...
            Vector3f.Add(ref Position, ref linearVelocity, out Position);

            //Update the orientation based on the angular velocity.
            Vector3f increment;
            Vector3f.Multiply(ref angularVelocity, .5f, out increment);
        ...
        internal void ApplyLinearImpulse(ref Vector3f impulse)
        {
            Vector3f velocityChange;
            Vector3f.Multiply(ref impulse, inverseMass, out velocityChange);
            Vector3f.Add(ref linearVelocity, ref velocityChange, out linearVelocity);
        }

        internal void ApplyAngularImpulse(ref Vector3f impulse)
        {
            Vector3f velocityChange;
            Vector3f.Transform(ref impulse, ref inertiaTensorInverse, out velocityChange);
            Vector3f.Add(ref velocityChange, ref angularVelocity, out angularVelocity);
        }
