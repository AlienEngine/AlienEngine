...
namespace AlienEngine.Core.Physics.Constraints.Collision
{
    ...
    public class ContactPenetrationConstraint : SolverUpdateable
    {
        ...
        public override void Update(float dt)
        {
            ...
            if (entityA != null)
            {
                Vector3f.Subtract(ref contact.Position, ref entityA.position, out ra);
            ...
            if (entityB != null)
            {
                Vector3f.Subtract(ref contact.Position, ref entityB.position, out rb);
            ...
            if (contact.PenetrationDepth >= 0)
            {
                ...
                if (contactManifoldConstraint.materialInteraction.Bounciness > 0)
                {
                    ...
                    if (bounceVelocity > 0)
                    {
                        ...
                        var velocityFraction = MathHelper.Clamp((bounceVelocity - lowThreshold) / (CollisionResponseSettings.BouncinessVelocityThreshold - lowThreshold + MathHelper.Epsilon), 0, 1);
