...
namespace AlienEngine.Core.Physics.Constraints.TwoEntity.Motors
{
    /// <summary>
    ...
    public class RevoluteMotor : Motor, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        public RevoluteMotor(Entity connectionA, Entity connectionB, Vector3f motorizedAxis)
        {
        ...
        public void SetupJointTransforms(Vector3f motorizedAxis)
        {
            //Compute a vector which is perpendicular to the axis.  It'll be added in local space to both connections.
            Vector3f xAxis;
            Vector3f.Cross(ref motorizedAxis, ref Toolbox.UpVector, out xAxis);
            float length = xAxis.LengthSquared;
            if (length < MathHelper.Epsilon)
            {
                Vector3f.Cross(ref motorizedAxis, ref Toolbox.RightVector, out xAxis);
            }
        ...
        public override void Update(float dt)
        {
            ...
            if (settings.mode == MotorMode.Servomechanism)
            {
                float y, x;
                Vector3f yAxis;
                Vector3f.Cross(ref basis.primaryAxis, ref basis.xAxis, out yAxis);
                Vector3f.Dot(ref worldTestAxis, ref yAxis, out y);
                Vector3f.Dot(ref worldTestAxis, ref basis.xAxis, out x);
