...
namespace AlienEngine.Core.Physics.Constraints.TwoEntity.JointLimits
{
    ...
    public class EllipseSwingLimit : JointLimit, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        private Vector3f localTwistAxisB;
        private float maximumAngleX;
        private float maximumAngleY;
        private Vector3f worldTwistAxisB;
        ...
        public EllipseSwingLimit(Entity connectionA, Entity connectionB, Vector3f twistAxis, float maximumAngleX, float maximumAngleY)
        {
        ...
        /// </summary>
        public Vector3f LocalTwistAxisB
        {
            get { return localTwistAxisB; }
            set
            {
                localTwistAxisB = value;
                Vector3f.Transform(ref localTwistAxisB, ref connectionB.orientationMatrix, out worldTwistAxisB);
            }
        }
        ...
        public float MaximumAngleX
        {
            ...
            set { maximumAngleX = MathHelper.Clamp(value, MathHelper.BigEpsilon, MathHelper.Pi); }
        }
        ...
        public float MaximumAngleY
        {
            ...
            set { maximumAngleY = MathHelper.Clamp(value, MathHelper.BigEpsilon, MathHelper.Pi); }
        }
        ...
        /// The transformed twist axis will be used to determine the angles around entity A's basis axes.
        /// </summary>
        public Vector3f TwistAxisB
        {
            get { return worldTwistAxisB; }
            set
            {
                worldTwistAxisB = value;
                Vector3f.TransformTranspose(ref worldTwistAxisB, ref connectionB.orientationMatrix, out localTwistAxisB);
            }
        }
        ...
        public void SetupJointTransforms(Vector3f twistAxis)
        {
            //Compute a vector which is perpendicular to the axis.  It'll be added in local space to both connections.
            Vector3f xAxis;
            Vector3f.Cross(ref twistAxis, ref Toolbox.UpVector, out xAxis);
            float length = xAxis.LengthSquared;
            if (length < MathHelper.Epsilon)
            {
                Vector3f.Cross(ref twistAxis, ref Toolbox.RightVector, out xAxis);
            }

            Vector3f yAxis;
            Vector3f.Cross(ref twistAxis, ref xAxis, out yAxis);
        ...
        public override void Update(float dt)
        {
            ...
            Vector3f axis;
            float angle;
            Quaternion.ToAxisAngle(ref relativeRotation, out axis, out angle);

#if !WINDOWS
            Vector3f axisAngle = new Vector3f();
#else
            Vector3f axisAngle;
#endif
            ...
            float angleX;
            Vector3f.Dot(ref axisAngle, ref basis.xAxis, out angleX);
            float angleY;
            Vector3f.Dot(ref axisAngle, ref basis.yAxis, out angleY);
            ...
            Vector3f sphereTangentX, sphereTangentY;
            Vector3f.Transform(ref basis.xAxis, ref relativeRotation, out sphereTangentX);
            Vector3f.Transform(ref basis.yAxis, ref relativeRotation, out sphereTangentY);

            Vector3f.Multiply(ref sphereTangentX, tangent.X, out jacobianA); //not actually jA, just storing it there.
            Vector3f.Multiply(ref sphereTangentY, tangent.Y, out jacobianB); //not actually jB, just storing it there.
            Vector3f.Add(ref jacobianA, ref jacobianB, out jacobianA);
            ...
            if (bounciness > 0)
            {
                ...
                Vector3f.Dot(ref connectionA.angularVelocity, ref jacobianA, out relativeVelocity);
                Vector3f.Dot(ref connectionB.angularVelocity, ref jacobianB, out dot);
