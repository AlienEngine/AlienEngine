...
namespace AlienEngine.Core.Physics.Constraints.TwoEntity.JointLimits
{
    ...
    public class RevoluteLimit : JointLimit, I2DImpulseConstraintWithError, I2DJacobianConstraint
    {
        ...
        private Vector3f jacobianMaxA;
        private Vector3f jacobianMaxB;
        private Vector3f jacobianMinA;
        private Vector3f jacobianMinB;
        private bool maxIsActive;
        private bool minIsActive;
        private Vector2f error;
        private Vector3f localTestAxis;
        ...
        private Vector3f worldTestAxis;
        private Vector2f velocityToImpulse;
        ...
        public RevoluteLimit(Entity connectionA, Entity connectionB, Vector3f limitedAxis, Vector3f testAxis, float minimumAngle, float maximumAngle)
        {
        ...
        /// </summary>
        public Vector3f LocalTestAxis
        {
            get { return localTestAxis; }
            set
            {
                localTestAxis = Vector3f.Normalize(value);
                Vector3f.Transform(ref localTestAxis, ref connectionB.orientationMatrix, out worldTestAxis);
            }
        }
        ...
        /// </summary>
        public Vector3f TestAxis
        {
            get { return worldTestAxis; }
            set
            {
                worldTestAxis = Vector3f.Normalize(value);
                Vector3f.TransformTranspose(ref worldTestAxis, ref connectionB.orientationMatrix, out localTestAxis);
            }
        }
        ...
        public Vector2f RelativeVelocity
        {
            get
            {
                if (isLimitActive)
                {
                    ...
                    Vector2f toReturn = Vector2f.Zero;
                    if (minIsActive)
                    {
                        Vector3f.Dot(ref connectionA.angularVelocity, ref jacobianMinA, out velocityA);
                        Vector3f.Dot(ref connectionB.angularVelocity, ref jacobianMinB, out velocityB);
                        toReturn.X = velocityA + velocityB;
                    }
                    if (maxIsActive)
                    {
                        Vector3f.Dot(ref connectionA.angularVelocity, ref jacobianMaxA, out velocityA);
                        Vector3f.Dot(ref connectionB.angularVelocity, ref jacobianMaxB, out velocityB);
        ...
        public override float SolveIteration()
        {
            ...
            if (minIsActive)
            {
                ...
                {
                    Vector3f.Multiply(ref jacobianMinA, lambda, out impulse);
                    connectionA.ApplyAngularImpulse(ref impulse);
                }
                if (connectionB.isDynamic)
                {
                    Vector3f.Multiply(ref jacobianMinB, lambda, out impulse);
            ...
            if (maxIsActive)
            {
                ...
                if (connectionA.isDynamic)
                {
                    Vector3f.Multiply(ref jacobianMaxA, lambda, out impulse);
                    connectionA.ApplyAngularImpulse(ref impulse);
                }
                if (connectionB.isDynamic)
                {
                    Vector3f.Multiply(ref jacobianMaxB, lambda, out impulse);
                    connectionB.ApplyAngularImpulse(ref impulse);
                }
        ...
        public override void Update(float dt)
        {
            ...
            Vector3f.Dot(ref minPlaneNormal, ref worldTestAxis, out planePositionMin);
            Vector3f.Dot(ref maxPlaneNormal, ref worldTestAxis, out planePositionMax);
            ...
            if (span >= MathHelper.Pi)
            {
                if (planePositionMax > 0 || planePositionMin > 0)
                {
                    ...
                    maxIsActive = false;
                    error = Vector2f.Zero;
                    accumulatedImpulse = Vector2f.Zero;
                    isLimitActive = false;
                    return;
                }
            ...
            if (minIsActive)
            {
                Vector3f.Cross(ref minPlaneNormal, ref worldTestAxis, out jacobianMinA);
                if (jacobianMinA.LengthSquared < MathHelper.Epsilon)
                {
            ...
            if (maxIsActive)
            {
                Vector3f.Cross(ref maxPlaneNormal, ref worldTestAxis, out jacobianMaxA);
                if (jacobianMaxA.LengthSquared < MathHelper.Epsilon)
                {
                    //The plane normal is aligned with the test axis.
                    //Use the basis's free axis.
            ...
            if (minIsActive)
            {
                ...
                if (bounciness > 0)
                {
                    ...
                    float dot;
                    //Find the velocity contribution from each connection
                    Vector3f.Dot(ref connectionA.angularVelocity, ref jacobianMinA, out relativeVelocity);
                    Vector3f.Dot(ref connectionB.angularVelocity, ref jacobianMinB, out dot);
            ...
            if (maxIsActive)
            {
                ...
                if (bounciness > 0)
                {
                    ...
                    {
                        float relativeVelocity;
                        Vector3f.Dot(ref connectionA.angularVelocity, ref jacobianMaxA, out relativeVelocity);
                        float dot;
                        Vector3f.Dot(ref connectionB.angularVelocity, ref jacobianMaxB, out dot);
                        relativeVelocity += dot;
            ...
            if (connectionA.isDynamic)
            {
                if (minIsActive)
                {
                    Vector3f.Transform(ref jacobianMinA, ref connectionA.inertiaTensorInverse, out transformedAxis);
                    Vector3f.Dot(ref transformedAxis, ref jacobianMinA, out minEntryA);
                }
                else
                    minEntryA = 0;
                if (maxIsActive)
                {
                    Vector3f.Transform(ref jacobianMaxA, ref connectionA.inertiaTensorInverse, out transformedAxis);
                    Vector3f.Dot(ref transformedAxis, ref jacobianMaxA, out maxEntryA);
                }
                else
                    maxEntryA = 0;
            }
            ...
            if (connectionB.isDynamic)
            {
                if (minIsActive)
                {
                    Vector3f.Transform(ref jacobianMinB, ref connectionB.inertiaTensorInverse, out transformedAxis);
                    Vector3f.Dot(ref transformedAxis, ref jacobianMinB, out minEntryB);
                }
                else
                    minEntryB = 0;
                if (maxIsActive)
                {
                    Vector3f.Transform(ref jacobianMaxB, ref connectionB.inertiaTensorInverse, out transformedAxis);
                    Vector3f.Dot(ref transformedAxis, ref jacobianMaxB, out maxEntryB);
                }
                else
                    maxEntryB = 0;
            }
        ...
        public override void ExclusiveUpdate()
        {
            ...
            if (connectionA.isDynamic)
            {
                ...
                if (minIsActive)
                {
                    Vector3f.Multiply(ref jacobianMinA, accumulatedImpulse.X, out impulse);
                }
                if (maxIsActive)
                {
                    Vector3f temp;
                    Vector3f.Multiply(ref jacobianMaxA, accumulatedImpulse.Y, out temp);
                    Vector3f.Add(ref impulse, ref temp, out impulse);
                }
                connectionA.ApplyAngularImpulse(ref impulse);
            }
            if (connectionB.isDynamic)
            {
                ...
                if (minIsActive)
                {
                    Vector3f.Multiply(ref jacobianMinB, accumulatedImpulse.X, out impulse);
                }
                if (maxIsActive)
                {
                    Vector3f temp;
                    Vector3f.Multiply(ref jacobianMaxB, accumulatedImpulse.Y, out temp);
                    Vector3f.Add(ref impulse, ref temp, out impulse);
                }
                connectionB.ApplyAngularImpulse(ref impulse);
            }
