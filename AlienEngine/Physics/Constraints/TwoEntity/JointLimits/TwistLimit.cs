...
namespace AlienEngine.Core.Physics.Constraints.TwoEntity.JointLimits
{
    ...
    public class TwistLimit : JointLimit, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        public TwistLimit(Entity connectionA, Entity connectionB, Vector3f axisA, Vector3f axisB, float minimumAngle, float maximumAngle)
        {
        ...
        public void SetupJointTransforms(Vector3f worldTwistAxisA, Vector3f worldTwistAxisB)
        {
            worldTwistAxisA.Normalize();
            worldTwistAxisB.Normalize();

            Vector3f worldXAxis;
            Vector3f.Cross(ref worldTwistAxisA, ref Toolbox.UpVector, out worldXAxis);
            float length = worldXAxis.LengthSquared;
            if (length < MathHelper.Epsilon)
            {
                Vector3f.Cross(ref worldTwistAxisA, ref Toolbox.RightVector, out worldXAxis);
            }
            worldXAxis.Normalize();

            //Complete A's basis.
            Vector3f worldYAxis;
            Vector3f.Cross(ref worldTwistAxisA, ref worldXAxis, out worldYAxis);
            ...
            Vector3f.Transform(ref worldXAxis, ref rotation, out worldXAxis);
        ...
        public override void Update(float dt)
        {
            ...
            Vector3f.Transform(ref basisB.xAxis, ref rotation, out twistMeasureAxis);

            //By dotting the measurement vector with a 2d plane's axes, we can get a local X and Y value.
            float y, x;
            Vector3f.Dot(ref twistMeasureAxis, ref basisA.yAxis, out y);
            Vector3f.Dot(ref twistMeasureAxis, ref basisA.xAxis, out x);
            ...
            if (error > 0)
            {
                Vector3f.Add(ref basisA.primaryAxis, ref basisB.primaryAxis, out jacobianB);
            ...
            else
            {
                ...
                Vector3f.Add(ref basisA.primaryAxis, ref basisB.primaryAxis, out jacobianA);
                if (jacobianA.LengthSquared < MathHelper.Epsilon)
                {
                    //A nasty singularity can show up if the axes are aligned perfectly.
                    //In a 'real' situation, this is impossible, so just ignore it.
                    isActiveInSolver = false;
