...
namespace AlienEngine.Core.Physics.Constraints.TwoEntity.Joints
{
    ...
    public class SwivelHingeAngularJoint : Joint, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        private float error;
        private Vector3f localHingeAxis;
        private Vector3f localTwistAxis;
        private Vector3f worldHingeAxis;
        private Vector3f worldTwistAxis;
        ...
        /// The connected entities will be able to rotate around this axis relative to each other.</param>
        public SwivelHingeAngularJoint(Entity connectionA, Entity connectionB, Vector3f worldHingeAxis, Vector3f worldTwistAxis)
        {
        ...
        public Vector3f LocalHingeAxis
        {
            ...
            set
            {
                localHingeAxis = Vector3f.Normalize(value);
                Vector3f.Transform(ref localHingeAxis, ref connectionA.orientationMatrix, out worldHingeAxis);
            }
        }
        ...
        public Vector3f LocalTwistAxis
        {
            ...
            set
            {
                localTwistAxis = Vector3f.Normalize(value);
                Vector3f.Transform(ref localTwistAxis, ref connectionB.orientationMatrix, out worldTwistAxis);
            }
        }
        ...
        public Vector3f WorldHingeAxis
        {
            ...
            set
            {
                worldHingeAxis = Vector3f.Normalize(value);
                Quaternion conjugate;
                Quaternion.Conjugate(ref connectionA.orientation, out conjugate);
                Vector3f.Transform(ref worldHingeAxis, ref conjugate, out localHingeAxis);
            }
        }
        ...
        public Vector3f WorldTwistAxis
        {
            ...
            set
            {
                worldTwistAxis = Vector3f.Normalize(value);
                Quaternion conjugate;
                Quaternion.Conjugate(ref connectionB.orientation, out conjugate);
                Vector3f.Transform(ref worldTwistAxis, ref conjugate, out localTwistAxis);
            }
        }
        ...
        public override void Update(float dt)
        {
            ...
            Vector3f.Cross(ref worldHingeAxis, ref worldTwistAxis, out jacobianA);
            float length = jacobianA.LengthSquared;
            if (length > MathHelper.Epsilon)
                Vector3f.Divide(ref jacobianA, (float)Math.Sqrt(length), out jacobianA);
            else
                jacobianA = new Vector3f();
