...
namespace AlienEngine.Core.Physics.Constraints.SolverGroups
{
    ...
    public class SwivelHingeJoint : SolverGroup
    {
        ...
        public SwivelHingeJoint(Entity connectionA, Entity connectionB, Vector3f anchor, Vector3f hingeAxis)
        {
            if (connectionA == null)
            ...
            baseAxis -= Vector3f.Dot(baseAxis, hingeAxis) * hingeAxis;
            if (baseAxis.LengthSquared < MathHelper.BigEpsilon)
            {
                ...
                baseAxis = Vector3f.Cross(hingeAxis, VectorHelper.Up);
                if (baseAxis.LengthSquared < MathHelper.BigEpsilon)
                {
                    baseAxis = Vector3f.Cross(hingeAxis, VectorHelper.Right);
                }
            }
