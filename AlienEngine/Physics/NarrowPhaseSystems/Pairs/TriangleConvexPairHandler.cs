        ...
        public override void UpdateTimeOfImpact(Collidable requester, float dt)
        {
                ...
                )
            {
                ...
                else if (triangleMode == PositionUpdateMode.Discrete)
                {
                    ...
                    Vector3f.Negate(ref convex.entity.linearVelocity, out velocity);
                }
                else
                {
                    //Both objects are moving.
                    Vector3f.Subtract(ref triangle.entity.linearVelocity, ref convex.entity.linearVelocity, out velocity);
                }
                ...
                if (minimumRadiusA * minimumRadiusA < velocitySquared)
                {
                    ...
                    if (GJKToolbox.CCDSphereCast(new Ray(convex.worldTransform.Position, -velocity), minimumRadiusA, triangle.Shape, ref triangle.worldTransform, timeOfImpact, out rayHit))
                    {
                        ...
                        {                
                            ...
                            Vector3f AB, AC;
                            Vector3f.Subtract(ref triangle.Shape.vB, ref triangle.Shape.vA, out AB);
                            Vector3f.Subtract(ref triangle.Shape.vC, ref triangle.Shape.vA, out AC);
                            Vector3f normal;
                            Vector3f.Cross(ref AB, ref AC, out normal);

                            float dot;
                            Vector3f.Dot(ref rayHit.Normal, ref normal, out dot);
