...
namespace AlienEngine.Core.Physics.NarrowPhaseSystems.Pairs
{
    ...
    public class MobileMeshTerrainPairHandler : MobileMeshMeshPairHandler
    {
        ...
        protected override TriangleCollidable GetOpposingCollidable(int index)
        {
            ...
            Vector3f terrainUp = new Vector3f(mesh.worldTransform.LinearTransform.M21, mesh.worldTransform.LinearTransform.M22, mesh.worldTransform.LinearTransform.M23);
            float dot;
            Vector3f AB, AC, normal;
            ...
            Vector3f.Subtract(ref shape.vB, ref shape.vA, out AB);
            Vector3f.Subtract(ref shape.vC, ref shape.vA, out AC);
            Vector3f.Cross(ref AB, ref AC, out normal);
            Vector3f.Dot(ref terrainUp, ref normal, out dot);
            if (dot > 0)
            {
